...
const dir = '/private/tmp/studio-v3-pure-logs'; fs.mkdirSync(dir,{recursive:true});
...
Promise.all([worker(),worker()]).then(()=>{fs.writeFileSync('docs/reviews/studio-v3/pure-summary.json',JSON.stringify(results,null,2)+'\n');console.log(`${results.filter(r=>r.code===0).length}/${files.length} PURE green`);process.exitCode=results.some(r=>r.code!==0)?1:0;});
