...
Changelog item: The step race keeps player figures and a neutral track visible while syncs are delayed, uses a placeholder for missing outfits, and gives zero steps zero progress.

1. PROOF: crew-capture-node-audit.mjs, steprace-live-browser-audit.mjs, leaderboard-honesty-audit.mjs | REACH: PENDING-DEPLOY. Crew > The Step Race. Production hydrateRace retains all five figures and neutral rails with a stale or unavailable snapshot, restores missing body/skull layers, and removes the 6% minimum from fresh recorded progress. Node routing and copy pass; browser pixels, computed geometry and baseline RED remain BLOCKED pending the operator command below. No version stamp changed.

Frozen step-race work order SHA256: `28f9c3f587012262bc03895fb2ad73fff9637ec94e766820cb1877798cfc3514`.

Data path: `renderFriends` in `js/app.js:12505` owns the closure-local Crew data. `paint` reads `window.__testFriends` or `social.listFriends()` at `js/app.js:13255`; assigning `window.data.friends` cannot reach it. `hydrateRace` at `js/app.js:13496` independently reads `window.__testRace` or `social.fetchStepRace(wk)` at line 13521. `fetchStepRace` in `js/social.js:811` sends GET `/steps/week` at line 814. `social.raceStanding` at line 867 supplies `rows`, reading `steps`, `you`, `race.yourRank` and local weekly steps. Race rendering reads `p.outfit`, `p.steps`, `p.seenAt`, `p.rank` and `p.playerId`; `comparisonsFresh` previously gated the entire `.track`, including its `.run` avatar. One stale timestamp therefore removed both from every lane. The fallback `p.outfit || defaults` also accepted an empty object, which `avatarLayersHtml` at line 6429 renders without any body/skull layers.

Server evidence: accepted GET `/friends` rows (`server/src/index.js:2720`) contain `profile: null` when no profile has uploaded; there is no top-level steps field, and nested outfit/steps then do not exist. GET `/leaderboard` filters `profile IS NOT NULL` at line 3113; partial uploaded profiles may return `outfit: null`, level 1 and zero badges/gear count at lines 3122 to 3126. GET `/steps/week` additionally requires this week's key, current race rules and `week_steps > 0` at lines 3196 to 3200, then returns positive `steps`, nullable `outfit` and nullable `seenAt` at lines 3338 to 3354. Neither leaderboard nor race contains a never-synced registration. `last_seen` alone does not prove a profile upload or current activity.

Disposition and deviations: the reproducible race failure is the freshness gate plus incomplete outfit fallback. It is not evidence that the live user's friends never uploaded. A never-synced, zero-step remote racer is impossible under the checked-in query, so the degraded fixture has a stale, previously uploaded partial racer with its real positive total and a separate never-synced friend excluded from race/leaderboard. No omitted friend is promoted into a ranked lane and no steps are invented. Stale comparisons remain unavailable, with visible disclosure and an unfilled dashed rail. Marker positioning stays inside the rail independently of the data-scaled fill. Zero and tiny counts are defensive Node renderer cases, explicitly not purported server responses. The existing fixture-routing guard was updated because its old assertion required stale avatars to disappear. Changes in the shared `js/app.js` are confined to race-row rendering; presence helpers and other Crew renderers are untouched.

Node RED command: `node tests/crew-capture-node-audit.mjs --source=/tmp/steprace-live-before-app.js`. The baseline file was copied from this checkout before editing. The realistic degraded case exits 1 with actual 0, expected 5 figure containers. This establishes the missing markup, not pixels or computed geometry. The browser guard measures decoded raster ink, visible avatar bounds and occlusion through the existing capture library, and track dimensions, opacity and fill width against the recorded totals. Rendered zero-size, transparent, hidden-art and fabricated-fill controls must fail.

BLOCKED browser command, for an operator-owned local demo page: `CREW_CAPTURE_CDP=http://127.0.0.1:9222 CREW_CAPTURE_URL="http://127.0.0.1:8765/?demo" node tests/steprace-live-browser-audit.mjs`. Running `node tests/steprace-live-browser-audit.mjs` here exits 97 because no operator browser was supplied. This sandbox cannot bind the required listener; none was attempted. The operator must run the same guard against the pre-fix and final checkout builds. No rendered RED or GREEN is claimed here.

Final local proof: `node tests/unit.test.js` exits 0 with `384 passed, 0 failed`; `node tests/leaderboard-honesty-audit.mjs` exits 0 with `LEADERBOARD HONESTY: 49 passed, 0 failed`; `node tests/crew-capture-node-audit.mjs` exits 0 with `CREW CAPTURE NODE: 16 passed; browser geometry UNPROVEN`. All 159 unique PURE entries were enumerated by evaluating the actual PURE initialization and additions in `tests/release-gate.mjs`, then each was executed. Initial result: 156/159 exit 0. After correcting this patch's browser audit output validation, target routing and pending claim correspondence, the three nonzero suites (`guard-hygiene-lint.mjs`, `submission-preflight-audit.mjs`, `r6-guards-audit.mjs`) each exit 0 on rerun, yielding 159/159. The full enumeration, initial receipts and final receipts are in `/tmp/steprace-live-advisory.md` and `/tmp/steprace-live-proof/`. No full browser gate, live server response or deployment was verified.
