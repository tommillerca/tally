...
Written before the note, which is the point. The lint finds a section by the
newest changelog number, so this one is inert until whoever cuts the release
moves the line under that heading.
