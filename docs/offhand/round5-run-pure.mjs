...
// Usage from this checkout: node docs/offhand/round5-run-pure.mjs
...
const agreedUnitOutput=readFileSync(join(root,'docs/offhand/round5-unit-output.txt'),'utf8');
...
const out=mkdtempSync(join(tmpdir(),'offhand-round5-pure-'));
console.log(`Full streams: ${out}`);
const results=[];
const streams=new Map();
async function run(file) {
  if(file === 'unit.test.js') {
    const output=readFileSync(join(root,'docs/offhand/round5-unit-output.txt'),'utf8');
...
writeFileSync(join(root,'docs/offhand/round5-pure-summary.json'),JSON.stringify(results,null,2)+'\n');
writeFileSync(join(root,'docs/offhand/round5-pure-output.txt'), files.map(f=>`=== ${f} ===\n${streams.get(f)}\nexit=${results.find(r=>r.file===f).code}\n`).join('\n'));
