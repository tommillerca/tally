...
// Usage from this checkout: node docs/r6-guards/run-pure.mjs
...
assert.ok(files.length>=151,`Refusing undersized PURE census: ${files.length}/151`);
assert.equal(new Set(files).size,files.length,'PURE registrations must be unique');
...
const out=mkdtempSync(join(tmpdir(),'r6-guards-pure-'));
...
writeFileSync(join(root,'docs/r6-guards/pure-results.json'),JSON.stringify(results,null,2)+'\n');
