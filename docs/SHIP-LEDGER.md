...
- **web**: `curl` the changed module on the live origin and grep for a symbol the
  change introduced. That is the check that matters.

  CORRECTED 2026-08-23. This file used to say "a merge alone reaches nobody when
  the module is in `sw.js` PRECACHE and `VERSION` has not moved." THAT IS WRONG
  and it cost real decisions: two sessions independently told Tom that merged
  fixes were reaching nobody, which mis-prioritised a release.

  Read the `fetch` handler in `sw.js`. The app shell — the HTML plus our OWN
  js/mjs/css/json, everything not under `/vendor/` — is served **NETWORK-FIRST**
  with `cache: 'no-cache'`, and the cache is the OFFLINE FALLBACK ONLY. So for
  app code, **merging to main IS shipping**: an online device picks it up on the
  next load. Being listed in PRECACHE does not pin a module.

  What the `VERSION` bump actually delivers: the `APP_BUILD` string Settings
  shows, the What's New dot, and a fresh precache set for offline use. Those are
  worth bumping for, and the bump is still part of the ritual — but it is NOT
  what puts code in front of players. Do not hold an urgent fix waiting on it.

  Cache-first, and therefore genuinely gated on the bump: fonts, images and
  anything under `/vendor/`.

  Verified 2026-08-23 by fetching `js/app.js` from the live origin and finding
  `gwCrateNagged` and `TAB_DBL` (merged in #98 that same hour) already present.
