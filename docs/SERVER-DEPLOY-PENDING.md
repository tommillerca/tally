...
## DEPLOYED 2026-09-12 14:57 Pacific (Tom: "d1 deploy now")

Migration `2026-09-12-spire-takeover-receipt.sql` applied to production D1 (spires now carries takeover_id, confirmed by PRAGMA before and after). `server/deploy.sh` then ran clean through its preflight (write contract, three secrets present, tree equals origin/main, schema-plan 45/45, API 87/87, retention 19/19, grants retention 23/23, remote schema guard) and published Worker version 23e80a11-8226-4d23-a2aa-71dae875f8f5, 100% of traffic, confirmed by `wrangler deployments status`. The script's own post-deploy deep-health curl hit the old version's 404 during rollout; re-checked one minute later: /health 200, /health/deep 200 ("write-schema", 12 tables), /steps/week 401, /steps/settled 401. Live now: atomic podium settlement, gift and cheer stable retry keys (retries answer with the original acknowledgement plus duplicate:true), spire takeover receipts, replay identity from the verified request, flagged test accounts suppressed from friends and from gift, cheer and accept, player-local race week keys. Two fixes were needed to get through the preflight: the friends route's suppression predicates moved into the JOIN conditions so the multi-index OR seek survives (schema-plan.test.mjs had also drifted since v498 on its bind order), and the retry acknowledgement regained duplicate:true.
