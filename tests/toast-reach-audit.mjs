 ...
 * Fixture diagnosis (Round 4): route() dispatches settings to renderSettings.
 * The audit's dismissOverlays() calls click() with a Claim label matcher
 * against ALL page buttons. Settings' #surveyBtn says Claim and its click handler calls
 * openSurveySheet('settings'). The v1 automatic survey prompt was removed.
 * This is a fixture click, not a routing takeover or a survey-due seed.
 * Do not dismiss by global labels between routes: that activates route actions.
 * CONTROL requires rendered AND reachable #screen controls, with no sheet
 * obscuring the route. Shell navigation or a survey cannot satisfy it.
  ...
  for (const [w, h] of [[375, 812], [430, 932]]) {
    ...
    for (const route of ['today', 'friends', 'foods', 'bonehead', 'shop', 'studio', 'progress', 'settings', 'boneyard']) {
      ...
      const m = await page.evaluate(async () => {
        ...
        // A positive global count could be only shell nav or an unrelated
        // overlay. Require actual route-owned controls, including tall stages.
        ...
        return { controls, sheets: document.querySelectorAll('#sheets .sheet').length, hash: location.hash, renderedScreenControls: document.querySelector('#screen')?.querySelectorAll(selector).length || 0, toast: { y: b.y, height: b.height }, visible: !toast.hidden && ts.visibility !== 'hidden' && +ts.opacity > 0 && b.width > 100 && b.height > 20 && b.left >= 0 && b.top >= 0 && b.right <= innerWidth && b.bottom <= innerHeight, policy: ts.zIndex === '320' && ts.pointerEvents === 'none' };
      });
      const label = `${route} ${w}x${h}`;
      ok('CONTROL', m.hash === '#/' + route && m.sheets === 0 && m.renderedScreenControls > 0 && m.controls.some(c => c.screen), `${label} hash=${m.hash} sheets=${m.sheets} count=${m.controls.length} screen=${m.controls.filter(c => c.screen).length} renderedScreen=${m.renderedScreenControls}${m.controls.some(c => c.screen) ? '' : ` reachable=${JSON.stringify(m.controls.map(c => c.name))}`}`);
