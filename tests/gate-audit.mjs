 ...
 *
 * PROVE-RED (confirmed 2026-08-07): change the spendPitFight call site's guard
 * back to `if (!spent)` and this exits 1 naming spendPitFight.
 *
 * PROVE-RED (confirmed 2026-08-15, the audit's OWN blind spot): rewrite
 * the Pit entry call to `const { ok } = await spendPitFight();` and drop the guard.
 ...
 * it understood NOTHING about was the spire claim's spend result, which is the
  ...
  lines.forEach((line, i) => {
    for (const fn of OK_HELPERS) {
      ...
      /* Look-ahead window MEASURED, not guessed: the spire claim declares its
         result and reads `r.ok` eleven lines later,
