...
if (!item || item === '2') {
  ...
  assert.equal(grant, 40, 'CONTROL read the production day-one grant');
  ...
  for (const note of notes) {
    assert.match(note, /\b40-coin starting wallet/, 'anchor rationale must name the real 40-coin starting wallet');
    assert.match(note, /cannot afford the 300-coin anchor/, 'anchor no longer meets starting-wallet affordability');
    assert.doesNotMatch(note, /340/);
  }
  assert.match(loot, /\[300, \['U2', 'U4', 'U7'\]\]/, 'CONTROL actual common rung price stays 300');
  console.log('PASS item 2: both comments disclose 40 coins cannot afford the unchanged 300-coin anchor');
}
if (!item || item === '5') {
