...
const DECLARED = {
  ...
  'xp-curve-model.mjs': ['skip', 'a reward MODEL, not a guard, same shape as garden-sim.mjs: four candidate Gauntlet XP curves plus a daily-cap option, costed at ranks 1 to 100 in XP per clear, levels per four first-clears and fights per level. It changes no game code, asserts nothing and exits 0 by design; it exists so the curve gets chosen on numbers. The guard for the difficulty half of the same problem is the ENDGAME block in balance.mjs.'],
