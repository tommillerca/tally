...
const BROWSER = [
  ...
  'dead-weight-audit.mjs',   // the other direction: a CSS rule, a function or an asset nothing can REACH. Pure node, about a second, and it belongs in FAST because dead weight in app.css and js/app.js is precached in full on first open, so it is bytes on a player's mobile data
...
const DECLARED = {
  ...
  'dead-weight-live-audit.mjs': ['full', "the live witness for dead-weight-audit: boots the app, walks every route, every hub tab, every sheet it can open two levels deep and a real Pit fight, and records every class and id that ever touches the DOM through a MutationObserver installed before the app renders. FAILS if anything the static sweep called DEAD is rendered, which is the one failure mode that matters when the static half's findings get deleted. Not FAST: it is a full browser drive, about three minutes."],
