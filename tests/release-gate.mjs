...
const DECLARED = {
  ...
  /* THE FAILURE-COPY SWEEP, 2026-08-17. Same family as log-write-failure-audit
     (which stays in FAST because it guards the single most common action), but
     it drives SEVEN failure paths, boots once and walks Today, Progress, Foods,
     the Backpack and Settings, so it is minutes rather than seconds. Its static
     half is the part that generalises: a new db.put on a player-data store that
     neither speaks on refusal nor carries a written exemption fails the audit
     before a browser starts. */
  'failure-voice-audit.mjs': ['full', "every failure a player can hit says WHAT happened and WHAT to do: seven driven paths (quick add, weigh-in, custom food, re-log, a crate whose row moved, a spent draught, a non-backup import file) graded on measured visibility, character and word FLOORS, a readability CEILING and a machine-string pattern, never on string equality. Plus a static coverage half over every db.put on log/weights/foods/health. Self-serves this checkout."],
