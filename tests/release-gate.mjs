...
const DECLARED = {
  ...
  'native-deadpath-audit.mjs': ['full', 'no toast raised in a NATIVE shell may name a web-only affordance (the registry is WEB_ONLY inside the file; today it is the file export and the file import, both of which dead-end in the WebView). Boots the real app twice, once with window.Capacitor.isNativePlatform() -> true, and grades the toasts the app actually raised, so a refactor that moves the string still fails. Carries the direction row too: the WEB shell must still offer the export tip, because deleting the tip everywhere is not the fix. Proven red at origin/main 56c5058 (NATIVE-CLEAN and NATIVE-SPOKE red, WEB-KEEPS green, exit 1). Two full boots with a 12s toast window each, so it is not FAST-shaped.'],
