...
const DECLARED = {
  ...
  'hollow-music-audit.mjs': ['full', "the Hollow's opt-in music, and above all its silence: a default profile must produce no play() call, no <audio> element and no stored true, checked three ways so one refactor cannot defeat it. Then the real pill under a real mouse: starts, stops, persists, still silent at boot, dies on close and on a hidden tab, and the Settings row kills the same one preference. FULL and not FAST only because it boots, reloads twice and opens the Hollow four times (~70s); the default-off property itself is the cheapest thing in here and it is the one that matters, so PROMOTE TO FAST if the music ever grows a second entry point."],
