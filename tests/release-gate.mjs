...
const DECLARED = {
  ...
  'event-queue-audit.mjs': ['full', "the analytics queue's admission policy: a REAL pit_win from a REAL fight survives a real 170-tap navigation burst that overflows QCAP, plus exact arithmetic, the live-shaped mix, the FIFO degenerate case and the __evProbe BOT gate. 63s, and two full boots of them, because the headline check has to win an actual fight before it has a rare row to protect. FULL rather than fast because what it guards is DATA QUALITY, not a player-visible break: a regression here silently under-reports engagement, it does not ship a broken screen. PROMOTE TO FAST if any dashboard number ever becomes a release criterion. Prove-red: revert either admit() call site in js/analytics.js to `q.slice(-QCAP)` and 7 checks go red while PRESSURE, FIFO, ORDER and BOT GATE stay green."],
