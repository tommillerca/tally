...
const DECLARED = {
  ...
  'map-retry-leak-audit.mjs': ['full', "the Boneyard may hold ONE MapLibre instance, not one per tap of its own Retry button. startMap() is bound to #mapRetry from both failure branches and used to reassign `map` with no teardown, so ten taps left ELEVEN live WebGL contexts (58 -> 493 listeners, 366 -> 2411 nodes, 5.55 -> 11.07 MB) and leaving the screen released one of them. Browsers ration live GL contexts near sixteen, so this breaks the map outright before the megabytes are alarming: a CEILING, not a trend (tally/CLAUDE.md rule 11). Aborts the tile host itself so the failure branch is deterministic on a networked box too, and needs the four swiftshader flags, which it passes. Nine startMap attempts at ~3.4s each plus a seeded boot, so it is FULL-shaped, not FAST. Prove-red is this file against a `git archive HEAD` tree at 56c5058: exit 1 with GL CEILING, RELEASED, LISTENERS and NODES all red."],
