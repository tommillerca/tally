...
const PURE = ['unit.test.js', 'first-session-audit.mjs', 'facegate-audit.mjs', 'garden-appetite-guard.mjs', 'pit.test.js', 'quest-daymore-audit.mjs', 'quest-pick-audit.mjs', 'first-fight-audit.mjs', 'analytics-tag-audit.mjs', 'icon-inventory-audit.mjs', 'version-stamp-audit.mjs', 'boneyard-supply-audit.mjs', 'loot-fallback-audit.mjs', 'guard-hygiene-lint.mjs', 'guard-provenance-lint.mjs', 'rack-theme-lint.mjs', 'pet-accessory-lint.mjs', 'pet-pool-audit.mjs'];
const BROWSER = [
  ...
  'nudge-skip-audit.mjs',    // "Not right now" belongs to the first-meal nudge, not to the banner under it. Tom, 2026-08-21: "for some reason it says 'not right now' right above the banner for the new creatures." Nothing was orphaned and nothing threw; the GROUPING was wrong. Measured on dafe778a at 393x852: the button's text sat 19px under its own card (12px of inherited .card margin-bottom plus its own 7px top padding) and 2px above the hype banner, so proximity said it dismissed the creatures. Graded in INK, not in boxes, because the whole bug lived in that padding and a rect-to-rect read would have called it 12 / 0. The bound has a DIRECTION (anti-regression rule 11): above must be SMALLER than below, below has a 12px floor (the app's own card gap) and the ratio a 2.5x floor, all three of which the shipped bug fails at both viewports. SETUP refuses to grade unless the exact stack is on screen (the nudge only renders for a save that has never logged food, so the audit forces that state) and unless something really is rendered below the pair. ALIVE drives the real button and asserts the choice was recorded, because the skip is the documented way out of the nudge and must never be deleted to fix the spacing. Proven red on the pristine parent commit and on three cp -R mutations, list in the file header
