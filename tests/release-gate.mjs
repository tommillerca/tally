...
PURE.push('prove-red-reference-lint.mjs'); // 2026-09-07: named files and quoted FAIL rows in audit prove-red headers must still exist
const BROWSER = [
  ...
  'first-session-lifecycle-audit.mjs', // R37-1/R39-4: both lifecycle installation sites are live. Drives real onboarding and a genuine resume across midnight, then reloads the installed save and repeats the midnight resume as a returning player. TAIL-1 goes red if enterAppFromOnboarding loses its bindAppLifecycle call; TAIL-2 goes red if boot loses its call. Pins headless 'new' because shell mode does not genuinely background a tab. ~12s, no screenshots.
