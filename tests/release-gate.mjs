...
/* kitchen-atomic-audit is PURE for the same reason (mem-idb under the real
   js/db.js and js/cooking.js, <1s): QA round 26 O2/O3/O4, two overlapping
   transmutes pay once, a moved clock is bounded in both directions, and two
   overlapping potion grants both land. Its two CONCURRENT rows are real here
   because mem-idb now serialises transactions the way the spec does. */
/* lapse-witness-audit is PURE for the same reason (mem-idb under the real
   js/db.js, js/game.js and js/social.js, ~4s of which 1.5s is the bounded wait
   it is measuring): round 33 LAPSE-1 and LAPSE-2, a returning player's owed day
...
/* wardrobe-family-audit is PURE for the same reason pet-accessory-lint is: it
   ...
   get a tile of their own and which are one tap further away inside a colourway
   rail. The catalogue is its whole input, so a browser would add cost and no
   coverage; the RENDERED grid is wardrobe-family-grid-audit's job below. Its
   APART row is the one that matters: twelve pairs measured as different
...
PURE.push('r4-app-p1-audit.mjs'); // Frozen R4 batch: purchases, forage abort/race, GPS expiry and future-day payout refusal.
PURE.push('water-retry-audit.mjs'); // R4-20: loader backoff and boot recovery, simulated clock/transport.
...
const BROWSER = [
  ...
  'first-session-lifecycle-audit.mjs', // R37-1: the install session gets a boot tail too. boot() returns early with no settings, straight into onboarding, and enterAppFromOnboarding used to rebind only the tab bar, hashchange and analytics: no onAppResume, no day-roll interval, no notif scheduling, no autoSync, for the whole of day one (round 37 handoff, measured 0 requests and 0 day closes across 3/3 backgrounds). Drives the REAL onboarding UI (no seeded settings, no reload, no calling enterAppFromOnboarding directly) to land on Today, walks a page-pinned fake Date across local midnight, fires a REAL visibilitychange resume (a real second tab backgrounded then foregrounded, same technique as multitab-audit.mjs), and asserts kv lastOpenDay reaches the new day -- writable only from bindAppLifecycle's onAppResume before boot() would ever run again. Pins headless 'new' over HEADLESS_MODE for the same reason multitab-audit does: 'shell' never truly backgrounds a tab, so visibilitychange never fires and a healthy app reads red. Proven red by deleting the enterAppFromOnboarding call site: TAIL alone fails (lastOpenDay stays null), everything else (onboarding itself) stays green. ~7s, no screenshots.
  ...
  'kennel-audit.mjs', // the Kennel: Tom's ruling 2026-09-05 put the morph-collection screen INSIDE the Stable (not a sixth Today door, not the Paddock -- off limits this phase); v500 feedback moved the way in from a button in the sheet head's dismiss corner to a DOOR in the Stable's body, which DOOR pins, and LEAD pins the one line that says how to read the grid. BUTTON pins the real button opening the real sheet; GRID pins exactly 30 cells (6 species x 5 morphs -- Bumbleseal counts as a normal species now, so this is the plan's original 25 plus her row); OWNED and UNOWNED pin that a granted pair draws unlocked-and-decoded while an ungranted one draws the locked placeholder decoded and NEVER a 404 (a silhouette, not a broken image); ROSTER pins one row per species actually granted through addPetInstance, not the whole catalogue; GWART pins the one sentence that states the cosmetic-only rule in plain language; FIT pins that all six roster rows sit above the fold with no scroll at BOTH 390x844 and 320x568, which is why Gwart's line sits below the roster and the row is 60px rather than the plan's original 72px (measured over budget at 320x568, 693px of a 568px screen, before the layout moved). Proven red in a cp -R copy, seven mutations, one row failing in each: a deleted #kennelBtn (SETUP, exits before grading anything), a 5-species grid, isOwned forced false, a 404'd locked-cell src, a roster read off the whole catalogue instead of ownership, the pre-measurement 72px/Gwart-above layout (320x568 only), and a swapped Gwart line. Three more rows 2026-09-07 (v500 feedback), each proven red the same way: ART x2 pins that no Kennel pet art is drawn above 1.4x its own source at 393x852 AND 320x568 (red at 1.821x on the hardcoded 192 tier, 25 of 30 cells over -- the narrow phone alone measured 1.396 and would have passed, which is why both viewports are graded), DOOR pins the entry is in the body and names itself, LEAD pins the grid's reading rule
  'pet-morph-audit.mjs', // per-species morph PNG variants (Kennel palettes, 2026-09-05, rewritten from Phase A's CSS-filter audit): CONTROL pins a base-morph instance's <img> resolves to the base master on Today's hero and the Stable card; TINT pins a coloured instance resolves to the real morph variant file AND that the decoded pixels actually differ from the base control; INK diffs the two real served files pixel-for-pixel under luminance 0.22 in the base file, max per-channel delta <= 2, so Cam's outline ink survives the recolor byte-identical; ACCESSORY pins Bumbleseal's worn layer never resolves to a /morph/ path and its pixels are unchanged whether her base morph is midnight or base. Proven red by making morphAsset always return '': TINT rows fail by name, pixel-diff rows read delta 0.00, CONTROL/INK stay green throughout (the exact "nothing was ever morphed" failure mode these rows exist to catch)
  ...
  'hype-banner-audit.mjs',   // the news pill's ONE featured hero banner. Tom, 2026-09-03: "we need to have one stand out banner that is showing more than a list of homework in the new section that gets people excited." RESTORED TO THIS TIER the same day it was moved out of it: the banner it grades was deleted that morning on a reading of "things will live in the collapsed news pill" as DELETE when it meant RELOCATE, and an audit retired for a surface that had merely MOVED is a guard switched off by a mistake. Re-premised at the hero slot: NEVER-EMPTY (art, copy, and exactly ONE hero, which is what fails if newsHero()'s fallback is ever deleted), IN-PILL (it is inside #newsBanner, and nothing promo-shaped is back on Today, with the hero itself as the positive control for that absence), ORDER (above the settled step race AND above the announcement rows: a hero under nine rows is row ten), ART (naturalWidth after an awaited decode, because an empty box measures perfectly, plus a 72px floor so it cannot slide back into the band of 40px thumbnails the rows use), FIT and COPY. Every measurement goes through a REAL TAP on the pill's summary first: a shut <details> is display:none, so without it every rect is 0x0. Both viewports, because the copy and the figures fail differently at 320 than at 393. The rows about two halves, two captions, their counts and the seal's box went with the two-half layout, which was the 2026-08-21 brief and is not this one
  ...
  'top-strip-audit.mjs',     // the top of the Bonehead screen is ONE colour, in the state that broke it. Tom, 2026-09-07, off his own phone: "ive noticed this top sliver recently a couple times sometimes it goes away i think after an update but looks glitchy" -- a black band a few tens of points down, the clipped amber top edge of a card inside it, and correct hero green above AND below. The cause is #updBanner, an in-flow sibling that is empty unless version.json says the live build is ahead of this one, which is the whole of the intermittency and why an update clears it: the hero's island bleed is a negative margin-top of --sat+14 that collapses out through .hero-card, so it lands the art at y=0 only while nothing above it has height. Measured at --sat 59 with the banner mounted, #updBanner 73 -> 152.9 and .hero-scene 91.9, i.e. the margin ate 73 of the banner instead of the scroller's padding and the hero painted over the rest. TOP is the geometry and INK is the colour, sampled 6px in from the left edge because the banner's own translucent fill hides --bg down the middle (at x=196 INK stayed green on the broken tree); BANNER is the positive control, so deleting the banner cannot pass this file. FAST because Today is the default screen and the stale state is the one every player passes through
  'returning-boot-audit.mjs',  // THE BOOT A LAPSED PLAYER GETS, which round 43 found is the one boot nobody had graded: the save came back perfect at 10, 30 and 90 days and the screen it came back to did not. Backdates a log row and kv 'lastOpenDay' by 90 days and RELOADS, so maybeWelcomeBack() is what decides the player is back and writes 'wbReturnDay' -- a stamped flag would grade a card the app might never raise. TOP photographs the day-close toast INSIDE the wheel-veil overlap (polled, not slept to) and grades the pill's own --surface-3 inside its own box: elementFromPoint is the wrong tool here because .toast is pointer-events: none on purpose, so a hit-test row could never pass and would have looked like a working guard. FOLD and WHOLE are the two halves of the 2026-09-05 trade that put the card below the day, graded at the two viewports it is a trade between, so neither old position can come back; FOLD asserts scrollTop 0 in the same evaluate as the measurement because 'at first paint' is a claim about where the screen was. TRUE grades the RENDERED card text: it must not claim everything survived while the streak reads 0, and it must name the streak. Proven red on the shipped tree, all four rows, with the numbers in the file header. FAST because a returning boot is the highest-stakes screen the app draws and every row here is a one-line revert. ~40s, 2 screenshots
  ...
  'race-profile.mjs',        // a leaderboard lane is a real button, opens THAT racer (level+class+badges read back, so name/index matching fails), your own lane is not a button, an existing friend gets a lane but no Add, and opening sends zero /friends/add. Written 2026-09-03 by the lane's own author and NEVER RUN by them; first run found its own openLane ripping the sheet out with .remove() instead of closing it, which left the app's sheetStack believing a sheet was open and made every tap after the first return {open:false} — a test bug that read exactly like a broken crew profile.
  'reveal-mannequin-audit.mjs', // every gear reveal draws the piece ON a base skeleton (B0-1 + SK0-1 + the awarded layer, all decoded) rather than as a loose PNG, with COVERAGE derived from js/app.js so a new bare-art card fails, and a byte ceiling that catches an unthumbed mannequin. This is what fixed the grillz reading as a 30px blob.
  ...
  'screen-sweep.mjs',        // no screen renders blank or throws, plus PRIVACY-LINK (R43-1): with kv 'surveyDone' forced TRUE it walks all seven routes for an anchor matching privacy|terms|legal|eula, requires the hit to be on Settings (no account, no survey), and FETCHES the href for a 200 whose body is the policy rather than the app shell, because sw.js answers a navigation miss with index.html and "the link exists" would pass on a 404. Proven red on origin/main: 0 matches across 7 routes
  ...
  'xp-cap-audit.mjs',        // a repeatable action cannot pay forever: CAP, ROLLOVER and CONTROL driven through the real awardCapped against a real IndexedDB. Landed with #29 and belonged to no tier. Self-serving, 16s, 11 checks, green on four consecutive runs of fc9bb0f
  ...
  /* THE FOUR FOOTBALL AUDITS MOVED FULL -> FAST ON 2026-09-04, THE DAY THE FLAG
     DID. Every one of them was in the FULL tier for the same declared reason and
     no other: FOOTBALL_KIT_LIVE was false, so nothing they graded could reach a
     player and ~310s of browser was the wrong price on every push. The flag is
     true now, the kit is on the lead shelf of the Shop every player opens, and
     an audit that quietly stops grading because its feature went live is the
     exact failure this repo keeps finding. football-render-audit's own entry
     said so in writing ("IT MOVES TO FAST THE DAY THE FLAG DOES"), and the other
     three each said "same tier and the same reason". Honoured here, all four. */
  'football-render-audit.mjs', // MOVED FULL->FAST 2026-09-04 with the flag. the football kit ON A SCREEN, which the PURE football-kit-audit cannot see. Three defects it exists for: the two lizard garments were NEVER DRAWN outside the shop (C4 and CX are both in ANIMATED_PETS, so petSpriteHtml returned animatedPetHtml before it could reach croppedPetImg, the only function that paints worn layers, and the kit was invisible on Today, the Stable, the Paddock and the Pit while the tint arithmetic was perfect); the static canvas the kit forces could change the animal's size; and VISOR_EYES_POLICY 'clip' either bounds the lasers inside the helmet or it does not, which is a fact about composited pixels and not about the URL the renderer emitted. EVERY measurement is a difference between two screenshots of the same rectangle of the same frame, never a getBoundingClientRect: PET-WEARS drives all three lizards (Beardie, its shiny, Day One) on two surfaces through a REAL tap on the Stable's wardrobe tile, PET-CONTROL is the negative (owned, worn by nobody, one animated stage and zero tint spans), PET-SIZE compares the ink with the garments hidden so it measures the animal rather than the outfit, PET-TINT renders each lizard under a navy team and a yellow one so the pixels that differ ARE the tinted ones and each render's mean has to be nearer its own team's primary, and PET-REGISTER splits the shiny (identical pixels, zero tolerance) from CX (whose ~2% drift is PREDICTED from PET_CROP and confirmed, so it is Cam's crop and not the renderer). VISOR-CLIP reads the helmet silhouette as ALPHA -- each layer shot alone over black and again over white -- after a first version that diffed against the page background reported 904 false escapes, because dark glass over the skull's dark eye sockets changes nothing; VISOR-SEEN keeps it from passing on a deleted eye layer and VISOR-CONTROL turns the mask off in the live DOM and requires the same measurement to report a real escape (0 clipped, 3804 unclipped). Animation is frozen for the size rows only, and the fly is hidden by name with the count asserted. Self-serving, ~150s, 12 checks
  'football-rail-audit.mjs', // MOVED FULL->FAST 2026-09-04 with the flag. the Wardrobe's colourway rail, the east-west slide through 32 team tints, which is four claims no source read and no getBoundingClientRect can answer. RAIL-EAST-WEST proves it is still ONE horizontal snapping line and not a wrapped grid (a grid renders 32 healthy tiles and 32 healthy rects; only their LAYOUT tells them apart). RAIL-CENTRE drives the real scroller for the FIRST, a MIDDLE and the LAST team and reads back what snapped, because `scroll-padding-inline: 50%` looked right, collapsed the snapport to zero width and landed a request for Windrow Wasps six teams away on Brightwater Barracudas with no error anywhere and every element healthy. RAIL-DOLL and RAIL-BOTH-STAGES measure the recolour as a difference between two screenshots of the same rectangle -- the paper doll and the rail's own figure each at their OWN pinned page-scroll position, because the two are 900px apart on a 932px screen -- and require the changed pixels' mean to move from nearer the OLD team's primary to nearer the NEW one; BOTH-STAGES exists because the first painter zipped a 4-long span list against a 2-long tint list BY INDEX, painted the doll, left the figure the player is actually watching on the old team, and reported 4 spans as if all was well. RAIL-STILL is the control that makes every diff mean something: the same rectangle twice with nothing touched, and it must be ZERO (it was 2935 until the rail's own scroll-behavior:smooth was pinned too, because the shots were landing mid-flight). RAIL-SCOPE keeps the helmet rail off the jersey on the same doll, RAIL-NOCOMMIT proves a slide across all 32 changes neither the worn item nor the coin balance, RAIL-COMMIT proves the bar's one control does, RAIL-LOCKED proves an unowned colourway still previews and still cannot be worn, and RAIL-COST pins the claim that 32 tiles cost 3 distinct image sources rather than 32 PNGs. Self-serving, ~35s, 11 checks
  'football-tile-crop-audit.mjs', // MOVED FULL->FAST 2026-09-04 with the flag. how a football garment is FRAMED in a tile, which is the defect Tom annotated on 2026-09-04 ("too zoomed in") and a defect no assertion about a CSS box can see: .fb-worn is the same 64x64 whether the art inside it is framed or blown off every edge. Measured: the helmet's own silhouette filled 94.2% of its square and ran off all four sides (L83 R45 T88 B46) while the jersey and cleats beside it sat at 42.6% and 33.4% inside theirs; one CSS declaration (.fit-fbhead, picked from a seven-scale sweep of the real tile) takes it to 43.4% and off no edge at all, and moves nothing else. THE INSTRUMENT IS THE POINT and three earlier ones are recorded in the file because each returned a confident number that meant nothing: hiding the whole mannequin measures the base skeleton (object-fit:cover, 98% of every tile, all five garments within 3 points); hiding just the garment and diffing measures the CONTRAST, so the same helmet read 61.3% in the shop and 20.7% on the wardrobe rail on two provably identical geometries; and reading an alpha with the tile's corners left rounded counts the corner arcs as garment (an identical L27 R28 T27 B27 on all three). What runs is the garment alone over a black ground and again over a white one, corners squared, so the number is the layer and not its surroundings. CROP-CLASS pins which frame each garment takes, CROP-INSIDE requires the helmet to touch no edge (the jersey's sleeve and the cleats' soles are ALLOWED to leave their frames: those are crops of a body), CROP-BAND requires it to fill about what the two garments Tom approved fill, CROP-CENTRED requires it centred within 5%, and CROP-CONTROL puts the old headwear frame back in the live DOM every single run and requires the same three measurements to report the defect, so the rows demonstrate their own failure rather than claiming it in a comment. It grades the WARDROBE RAIL's tiles because FOOTBALL_KIT_LIVE is false and the shop shelf does not exist for any test that does not edit the data file; they are the same construction taking the same one declaration. Self-serving, ~35s, 6 checks
  'shop-lead-order-audit.mjs', // MOVED FULL->FAST 2026-09-04 with the flag. WHICH SHELF LEADS THE SHOP, measured as boxes on a screen rather than as an order of substrings. Tom, 2026-09-04: "nfl shit goes to the lead shelf of the shop", and Bumbleseal moves down to the drop shelf beside the Puffer Pack. That is two orders across one flag, and the one that matters is the OFF one: FOOTBALL_KIT_LIVE is false in the shipped tree, so every player alive is looking at it and a mistake here breaks the Shop for all of them in order to ship a kit nobody can buy yet. LEAD-OFF therefore asserts the pet shelf is the FIRST child of #chContent, above the rack strip, with no #fbSect and no football markup anywhere in the container; LEAD-ON asserts the Kit room takes that place once the flag is open; DROP-ON asserts exactly ONE pet shelf, inside #shopRestBody, after #dropSect, with the Puffer Pack still present (Tom has not ruled on it and a shelf silently eaten by a re-order is the failure this row exists for). SIZE-ON answers "resized as needed" with the number: her hero measured 398x211.6 in the lead and 398x211.6 in the drop slot, so nothing needed resizing, and the row goes red if a future edit distorts her on the way down. THE FLAG IS NEVER FLIPPED ON DISK: data/football-teams.js is intercepted at the network for the ON half, the interception is COUNTED, and the page is made to import the module and report the flag back, because the first draft had no counter and an override that never fired produced "the flag-off shop looks like the flag-off shop" -- true, and worth nothing. Every rect is required to be in the live document, painting, and inside the viewport before anything is claimed about where it sits. Proved red three ways on a throwaway tree (gate dropped, gate removed, slots swapped), all exit 1, the FAIL lines quoted in the file. tests/unit.test.js carries the PURE half, which evaluates the same three shelf constants out of app.js and pins the order of the string. Self-serving, ~90s, 5 checks
  'kitchen-welcome-audit.mjs',    // R38-21: the welcome kit toast names Bone Broth and says Cook it, on a real fresh install
  'pit-kitchen-hint-audit.mjs',   // R38-22: the Pit names the active dish buff, or points at the Kitchen when there is something to cook
  'kitchen-day-one-strand-audit.mjs', // R38-23: a mis-tapped day-one pot can be cancelled and the ingredients come back
  'backup-lifecycle-audit.mjs',   // R38-2: a hidden/pagehide transition pushes the backup (real Chrome, mock Worker); red with the onAppHide wiring reverted
  'sheet-doubletap-audit.mjs',    // R39-12: two taps 60 ms apart on a sheet trigger leave the sheet open; a backdrop tap at 400 ms still closes it
  'pit-exit-motion-audit.mjs',    // Tom, live v487: the Pit exit stuttered; #pitBody was rewritten twice on close (16 ms and 262 ms)
  'crew-slime-leak-audit.mjs',    // 2026-09-06, Tom on v476: the viewer's slime glow lit every friend's shoes; the gift card listed every sealed gift
  'shop-door-audit.mjs',    // round 36, HANDOFFshoplocation20260905.md: "zero affordances anywhere outside the character hub say shop, buy, spend or store", and the two currency lines told a broke player what they could not afford and never how to earn more. Five fixes, one file: the Today teaser banner's door button (DOOR-*, and DOOR-ABSENT is its own finding -- cosmeticTeaserBannerHtml is unreachable from a real boot, same as its gardenBannerHtml/spireBannerHtml siblings, ever since the "Out there today" card was retired 2026-08-21, so the button and its delegated click listener are graded through the same webdriver-only __teaserBanner hook __todayRow already uses for bestiaryBannerHtml, not through a live Today surface that no longer exists); the hub remembering the last tab you were on (HUB-MEMORY: Shop, Today, Bonehead used to always land on Wardrobe); the coin wallet line routing to Today (WALLET, the same treatment the dust line already had); afford.coins counting owned pieces out of the pool (COUNT: "buys 21 of 21" stayed literal after buying one of the 21); and the coins-only Crew gift card's "Spend it in the Shop" line becoming a real tap target (GIFT). GIFT's own fix lives one level down from where it looks: the reveal card's pointerCapture starts on pointerdown, and capture retargets the compat click event to the capture element, so a listener on the button itself never received it -- the same failure mode the surrounding code already had a comment about for the card as a whole. Fixed at pointerdown, before capture is taken, not by guessing at the click handler. Self-serving, ~35s, 19 checks. Proven red on the unfixed tree (7b4d0492): 9 FAILED, one per surface -- DOOR-HOOK/DOOR-SHAPE, HUB-MEMORY, both WALLET rows, both COUNT rows, and GIFT
  ...
  'wardrobe-feedback-audit.mjs', // R9: 50 declared rows, real equip controls, slot/doll pixels, immediate receipts and deletion consent. Round 2 completion pending browser access.
  ...
  'today-idle-cpu-audit.mjs',  // QA round 26 O5: one logged food left Today at 60 style recalcs/s and 15 to 22% of a core until the player changed tab. `.hero-scene.bounce .bh-anim` listed bhbounce AND an infinite bh-idle on one `transform`, which Chrome never composites and never re-decides (the .wz-glow trap, idle-perf-audit item 3), so the idle ran on the main thread until the next innerHTML rebuild. Drives a real log through the portion sheet and reads CDP RecalcStyleCount over a 3s window 5s later: AFTER < 5/s (broken ~60/s), BEFORE as the quiet baseline, METER with an injected box-shadow loop so a zero is a measurement, LOGGED and BOUNCED so it cannot pass by logging nothing or by deleting the celebration, ALONE for the mechanism (one running animation, .bounce gone). Static half is unit.test.js R26-O5. Written in a no-browser lane: expected red on 96c1104a is AFTER ~60/s and ALONE 2 animations, everything else green; one boot, ~25s
...
const DECLARED = {
  'egg-progress-audit.mjs': ['full', "Manual walk CONTROL, PROGRESS, CAP, NO-RESET, RACE and rendered threshold TOLD; serves its own tree."],
  ...
  'settings-shape-audit.mjs': ['full', "Mobile Settings card preservation/order, nonempty controls, centre hit tests, folds and button shape at 393x852 and 320x568. Render proof blocked in sandbox; see docs/v580/guard-red.txt."],
  'backpack-wardrobe-f1-f2-audit.mjs': ['full', "Backpack content centring and Wardrobe rail order, nonempty pixel controls at both mobile sizes."],
  'settings-rows-audit.mjs': ['full', 'Settings 375x812 at 100%/200%: ROWS, signed-out PLUMBING and nonempty YOUR DATA CONTROL. COLUMN intentionally retired: content-sized actions replace fixed widths; settings-shape-audit NO-COLUMN guards stacked/clipped labels. Before/after measurements and RED proof BLOCKED by sandbox listen EPERM; no measured pass claimed. See docs/v577/guard-red.txt.'],
  ...
  'no-debug-markers-lint.mjs': ['fast', 'no shipped file (index.html, sw.js, app.css, js/, data/) carries a "do not commit" marker, a TEMP DIAGNOSTICS block or an id="dbg" sink. Node-only, sub-second. Added 2026-09-06 after v475 shipped an on-screen console.error overlay to every player.'],
  'crew-slime-leak-audit.mjs': ['full', 'the viewer\'s slime glow stays on the viewer\'s own Bonehead (zero .bh-slimed layers inside the Crew deck while the hero shows one), and six sealed gifts render as one button plus a count. Proven red on v476: 2 friend layers lit, 6 buttons.'],
  'kitchen-welcome-audit.mjs': ['full', 'a real fresh install through real onboarding sees the welcome kit toast name Bone Broth and say Cook it (R38-21: the instruction sat behind a branch that never ran). Red with the copy reverted.'],
  'pit-kitchen-hint-audit.mjs': ['full', 'renderPit carries one derived line: the active dish buff in plain words, or a pointer at the Kitchen when ingredients or a dish are owned, nothing when there is nothing to cook (R38-22). QUIET row grades the absence.'],
  'kitchen-day-one-strand-audit.mjs': ['full', 'the day-one wallet cooks the wrong pot (Stoneskin), confirms the strand (0 of 13 buttons), then the real Cancel refunds the ingredients; the day-one banner names the recipe the kit is for (R38-23).'],
  ...
  'tribute-collect-audit.mjs': ['full', "R20-P1: a tribute collection is a REAL TAP on #mapSpire with a pageerror listener live for the whole run, because three rounds closed that ticket by reading the import block instead. Measures the payout too (coins and dust both rise by exactly what is owed, and collectedAt advances), since an absence of errors is not a payout, and carries a CONTROL row that throws a ReferenceError into the same page so a green TAP cannot mean the drive never reached the handler. FULL, not FAST, for the same reason every other map row is: the only route to the button is a spire marker on a rendered Boneyard, so it needs WebGL and a reachable vector tile host and reports UNPROVEN (exit 97) without them. Proven red on 00979897, the tree R20-P1 was filed against: 2 FAILED, pageerror \"ReferenceError: boneDustAdd is not defined\", dust +0 while the day was still spent."],
  ...
  'clock-trust-audit.mjs': ['full', 'ASSERTS the monotonic day guard and MEASURES what is left. Installs a Date shim before any app module runs, and owns a loopback /health so the real API is never contacted. Asserts: a never-visited day below the high-water mark pays zero of every daily gate (quest coins, quest claims, free Pit fights, day-close crate, all-quests bonus) with any coin/crate movement attributed to a level-up or it is a leak; an honest forward day more than 20h later still pays the full day; that an idle month banks no allowance (rule 3; rule 2 was retired in QA round 26 O10 because it could not fire); that the evening-then-morning player, the eastbound traveller, both NTP corrections and an existing player mid-migration are none of them refused; that a 14-day forward clock walk pays ZERO of those same gates past witness + WITNESS_GRACE while every day inside the allowance still pays in full; and rule 3 on its own terms against a loopback /health it owns (the wire, monotonicity, both sides of the offline allowance, the heal, and that neither import path can lower the ceiling). FINDINGs, not assertions: the pre-computable quest rotation, the per-ENTRY food-logging XP, and the redeem-code one-shot. Fails on an empty sample set. About 60s.'],
  'pet-growth-audit.mjs': ['full', 'F7: Today lineage 0 versus 3, width and aspect ratio. Commander local browser and live RED equal-width proof pending.'],
  'crew-fan-audit.mjs': ['full', 'the Crew fan acceptance suite, 42 checks, about two minutes.'],
  'crew-layout-audit.mjs': ['full', "round 35's Crew tab pass, four tickets, 12 checks, about 20s: a sealed gift on the first 390x844 screen (CREW-7), the WORTH ADDING card collapsed to one row plus a working see-more (CREW-8), the fan's side card hit-testable instead of shadowed by the featured card on top of it (CREW-9), and a cheer phrase with an apostrophe reading as an apostrophe rather than the literal entity toast()'s textContent never decodes (CREW-10). Proven red on the pre-fix tree via `git stash push -- js/app.js app.css`: GIFT lands at 1515-1575px, HIT reads the featured card, the test seam for the toast fix does not exist yet, and WORTH opens on five rows with no more control."],
  'crew-activity-audit.mjs': ['full', "round 35's remaining Crew tickets, 17 checks, one boot, about 20s: a friend's card says nothing new only when nothing changed and names the one thing that did -- leveled up, new gear, a spire taken -- from a cached prior snapshot (CREW-4), a compact 'holds N spires' line on the friend card and profile with the one-line how-to-take-one (CREW-13), a stranger/friend whose `stats` synced as `{}` gets the honest 'will show once they next open the app' note and no Battle button that would otherwise hand the Pit a foe with no real numbers (CREW-14, the 'Jab ~NaN dmg' finding), the News race row saying the purse pays five (not three, CREW-6), and the Crew badge firing for the player's OWN rank improving overnight, not only for what other people did (CREW-5). Seeds __testMe/__testFriends (crew-fan-audit's pattern) and a new __testRace fixture (mirrors __testLb) so raceRankImproved and the race card's copy are checkable with no live account. Proven red per-ticket by reverting one fix at a time: dropping the sinceMap wiring drops CREW-4's label entirely and leaves CREW-13's persistent spire tag un-suppressed; reverting the stats guard shows a Battle button and 5 bars for `stats: {}`."],
  ...
  'wardrobe-rail-fit-audit.mjs': ['full', "the variant rail that opens under a tapped Wardrobe piece fits on screen and its cards are one size. Tom, 2026-09-12: 'this menu pops down below the fold and not near the gear youre trying to click it feels like a glitch. also youve fully miscaled the items'. Measured at 393x852 with 14 cosmetic families and 20 gear pieces seeded: the rail opened at y578 under a tile ending y577, correctly adjacent, but 303px tall so it ran to y880 on an 852px screen and the controls for the piece just tapped sat off the bottom. Cause of the height: the cosmetic cards carry a canvas.ward-art as a DIRECT child of the card button, while the only rule sizing it is `.pw-item.famr .famr-art canvas.ward-art`, which needs a .famr-art WRAPPER; the selector never matched and the canvas fell back to its intrinsic 200x200 inside a 104px card, 267px tall against 110px for the gear cards beside it. PRE-EXISTING, checked rather than assumed: the same fixture against app.css at d29682d1 produced the identical numbers. Rows CONTROL, NEAR (rail within 24px of the tapped tile), FITS, UNIFORM and ART. The fixture opens a GEAR tile specifically, because a family tile opens a two-card rail with no cosmetic card in it and the first version of this audit settled for one and stayed green with the fix removed. Proven red by deleting the canvas rule: FITS 578..856.5 in 852, ART 8 canvases at 200px in 104px cards. ~50s."],
  ...
  'toast-sheet-audit.mjs': ['full', "a toast must not land on a sheet's controls. R41-20: the welcome-kit toast fires 1.2s after "
    + 'onboarding and a new player is five taps from their first fight, so it arrived on the move tray. Unfixed at 375x667 it covered SIX '
    + 'move buttons, Jab and Bone Spike whole (toast 26.3,466 322.5x105). Fixed generally rather than for the Pit, because every sheet in '
    + 'this app puts its controls at the bottom and .toast sits 96px off the bottom: one app.css rule seats the toast under the sheet head '
    + 'while ANY sheet is open. Grades the REAL toast (the same webdriver-only __toast seam toast-map-audit drives) with the real '
    + "welcome-kit string, against every move button, the End Turn row and the sheet's own Flee button, at 393x852 and 375x667, plus SEAT, "
    + 'the regression the fix could most easily cause: the override leaking and top-seating every toast in the app. Proven red one mutation '
    + 'per throwaway copy: rule deleted (CLEAR 375x667, six overlaps), rule unconditional (SEAT 697.8px instead of 96). The 393x852 CLEAR '
    + "row is a fence, not a reproduction: see the file's HONEST LIMIT. Two real fights in one boot, ~50s."],
  'new-cosmetic-mark-audit.mjs': ['full', "R39-25: the cosmetic you just won can be FOUND again. v487's claim toast says what you won; "
    + 'nothing afterwards said which of the fourteen slots it landed in. Every grant now writes nw:1 on the inv row it was already writing '
    + "(cosRow/gearRow), and the Wardrobe renders it as a dot in the app's own unread language on the paper-doll slot rail and on the tile, "
    + 'cleared by clearNewInSlot() when that slot\'s grid is opened. On the row rather than a kv seen-timestamp because a timestamp needs a '
    + 'backfill answer for existing accounts and the honest one is a wall of dots. Grants through the REAL grantCosmetic into a slot the '
    + 'Wardrobe does not open on, then drives the two taps a player drives. MARK wants the dot on that slot and NOT on the other thirteen; '
    + 'TILE wants it on the tile; CLEAR and DURABLE want both gone on the next render and after a full reload, with the grid still '
    + 'populated so an empty grid cannot pass as quiet. Proven red on a throwaway copy with app.js/loot.js/app.css restored to the parent '
    + 'commit: MARK and TILE red, CONTROL green. One boot, 393x852, ~35s.'],
  'streak-card-audit.mjs': ['full', 'R41-21: a later streak milestone must never be a THINNER screen than an earlier one. Measured at 393x852 on '
    + 'the shipped card, day 7 carried three content blocks and day 14 carried two, because BADGES has streak-3/7/30 and nothing at 14, so '
    + 'with the number stripped day 14 said nothing day 7 did not. Fixed with a per-milestone line and a row of counts the app already '
    + 'holds, on EVERY milestone rather than bolted onto 14; a streak-14 badge would also have squared it and was not taken because a badge '
    + 'pays XP and is an economy change. Drives the SHIPPED openCelebration through a webdriver-only __celebrate seam with the badge '
    + "payload the real badge sweep would hand it. BLOCKS is a floor, SUBSET strips digits so '14 days' cannot pass as new content, and "
    + 'TRUE reads every number on the card back against ownedCosmeticIds/petInstances/earnedBadgeIds in the same page (no fabricated '
    + 'numbers). CONTROL goes red if a streak-14 badge is ever added, which is a signal to re-read this file, not to delete it. Proven red '
    + 'against the parent commit: BLOCKS, SUBSET and TRUE red, CONTROL green. One boot, ~20s.'],
  ...
  'wardrobe-family-grid-audit.mjs': ['full', "the collapsed grid, in pixels, on the collection F6 measured: all 57 hats owned at 390x844. THE \"BEFORE\" IS BUILT AND MEASURED, NOT DERIVED -- a flat .ward-grid of one .ward-cell per owned item (which is exactly what `items.map(...)` emitted before this change) is inserted in the LIVE document beside the real one, measured with the same getBoundingClientRect, and removed; both grids are asserted connected with non-degenerate boxes before either number is used, because ceil(n/4) x pitch is arithmetic about a stylesheet and this project has burned review cycles on that kind of estimate. COUNT is graded against the page's own bhFamilies over the owned set rather than a number typed into the file. SAMPLE is two controls in one: at least five COLLAPSED tiles and at least five PLAIN ones, so a badge that rendered on everything fails SOLO and a rule that collapsed nothing fails SAMPLE. WORN is the promise Tom asked for and it is graded on the tile canvas's PIXELS before and after equipping a sibling out of the rail, never on its data-art attribute, with an inked-pixel floor so a blank redraw cannot pass as \"it changed\". RAIL hit-tests every variant with elementFromPoint after scrolling the rail, so a rail that rendered at zero height or under a lid cannot pass. Toasts are removed and every animation paused before each shutter. One boot, 390x844, ~40s FULL rather than fast because the seed grants all 57 hats one at a time through the real grantCosmetic and the run drives four separate taps: measured ~100s, which is multiples of every suite in the fast tier."],
  'dressing-room-audit.mjs': ['full', "the Dressing Room's look picker collapses to one tile per bhFamilyKey with the fit grid's own colourway rail (famRailHtml with data-look), and \"Its own look\" is the gear's own art. Tom, v476: \"down in the transmog mirror section it's individually listing every single cleat/shirt etc as its own thing without a colour picker\". FAMILY grades the tile count against the page's OWN bhFamilies over the offered looks with a control that there is something to collapse; RAIL opens the rail from a real tap and wants one tinted, priced, non-zero-box tile per colourway; PICK taps a team off the rail and reads the After figure's tint span against the team table, then Wear it against the transmog map; OWN x5 disguises statted gear in H/T/P/S/FW, taps its own look and wants the gear's art file on the After figure and the stage, never the default and never nothing. Proved red on v477 (35 tiles for 33 looks, no family tile) and by mutation of previewEq's own-look branch (layers [] want FW/FW1.png). PICK's bar row reads .mog-dock > .mog-bar and wants the fit rail's .fb-bar still present: red on the bare $('.mog-bar') restageLook used until 2026-09-06 (dock bar stale, fit-rail bar false). One boot, 393x852, ~3 min, which is why it is full."],
