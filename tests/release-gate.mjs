...
const DECLARED = {
  ...
  'crate-palette-audit.mjs': ['full', "the common crate's nine authored 48x48 frames render their OWN colours and nothing else. Screenshots the reveal at the sprite's rect and asserts >= 99% of sampled device pixels are exact members of a palette derived from the PNGs at run time (45 distinct opaque RGB values, no hardcoded list), sampling only pixels whose source texel is opaque, about 1.33M device pixels over 14 samples at six profiles. Every other check on this sprite grades a BOX, and a box can be perfect while the art arrives as mush: this one is proven red, separately, against bilinear resampling (11.7%), the bloom over the outline (0.06%), a fractional opacity (0.00%) and a 1.2deg rotation (87.6%), and the rotation case leaves every geometric row green. It found the real bug it was written to find: .sheet's translateX(-50%) is a half CSS pixel at an odd viewport width, which cost 5.2% of the sprite at 375x812@3 and 393x852@3. Six viewport changes with a reveal opened at each plus full-frame PNG decodes, so it is about ninety seconds: not FAST-shaped."],
