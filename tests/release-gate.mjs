...
const DECLARED = {
  ...
  'reduced-motion-settle-audit.mjs': ['full', "prefers-reduced-motion must REMOVE motion, not add it. The reduce block collapsed transition-duration and left transition-property at its initial value `all`, which manufactured a live 1ms transition on every property of every element, including ones the designer gave none: 4 kinds of transition with the setting off, 17 with it on, 13 of them reduce-only. Anything that reveals or moves something and then MEASURES on the same task then reads the in-flight value, which is how a passing check goes latently flaky. Asserts the PROPERTY (a reveal hands the next reader a settled value) and not the CSS line, over five real app class flips plus a real route walk, with a CONTROL row proving the probe can see an in-flight read at all. Three boots at ~64s, so not FAST-shaped. Proven red in two throwaway trees whose 0.001s is dialled to 2s, one with the fix and one without, so the fix is the only variable and the race is observable rather than a 1ms coin flip."],
};
