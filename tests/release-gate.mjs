...
const DECLARED = {
  ...
  'sheet-pixel-grid-audit.mjs': ['full', 'a centring transform whose translation lands on a fraction cannot be pixel-snapped, so the layer rasterises off the grid while every getBoundingClientRect reads perfect. Measured on the real pixel crate at 100.00% -> 94.74% on-palette (393x852@3) with only the centring changed. Guards .pack-deck, whose --pc-w is min(61vw, ...) so its translateX was fractional at EVERY shipped width, both parities. PHASE reads the resolved matrix live at 5 widths x 2 DPRs and enumerates position:fixed descendants (a transform is their containing block, so dropping it re-anchors them); PIXELS proves on a fixture that the fraction really does change the raster, with an A/A control first because the takeover is an animation that scores 94.81% against ITSELF. About 2 minutes. Prove-red is NUDGE=0.5: PHASE and PIXELS go red, exit 1, A/A stays clean.'],
