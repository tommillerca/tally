...
const BROWSER = [
  ...
  'reward-sop-audit.mjs',    // every paying action against the rewarded-actions SOP: COVERAGE derives the paying call sites from js/*.js (158 sites in 53 actions across 44 modules) so a NEW payout nobody registered fails, UNDRIVEN prints the 34 registered-but-not-driven actions with their reasons every run, and REPEAT performs 20 actions twice against a real IndexedDB, sequentially AND concurrently. Self-serving, measured 20s, 72 checks, green on four consecutive runs of ba71522. Fast rather than full because it is the guard for the class that has now shipped three times, and every one of its ten reintroductions was proven red
