// R6-E2, frozen work order 2026-09-09. Register the previously isolated guard.
// PROVENANCE: docs/r6-merge/baseline.json and before.txt record five positive
// defect assertions on 3a403423999f542ee08cb4758d619f946e3c9c6f, exit 1.
// Real purchases, imports and encrypted CAS; in-memory IDB and transport only.
// No claim about visible notices, file-restore copy or concurrent-tap UI.
...
// The gate executes both opening balances in separate processes and databases.
// The small wallet catches a refusal before the compensating refund is applied.
if (!before && !process.argv.includes('--scenario') && !lowWallet) {
  for (const args of [[], ['--low-wallet']]) {
    const child=spawnSync(process.execPath,[fileURLToPath(import.meta.url),'--scenario',...args],
      {encoding:'utf8',timeout:60000,maxBuffer:4*1024*1024});
    process.stdout.write(child.stdout || '');
    process.stderr.write(child.stderr || '');
    assert.ifError(child.error);
    assert.equal(child.signal,null);
    assert.equal(child.status,0,`two-device scenario ${args.join(' ') || '100000 coins'} failed`);
  }
  console.log('PASS r6-merge: both opening wallets, every scenario completed');
  process.exit(0);
}
console.log(`SETUP real two-device scenario: opening wallet ${lowWallet?25000:100000} coins`);
...
offline=false;
// CONTROL: both devices really paid, distinct receipts exist, and every store
// has incoming data. Empty samples cannot satisfy the survival assertion.
assert.notDeepEqual(kv(forks[0]).labIncubators,kv(forks[1]).labIncubators);
for (const fork of forks) for (const store of ['log','foods','weights','inv','health','xp']) {
  assert.ok(fork[store].length>0,`missing sample in ${store}`);
}
const assertRowsSurvive=(incoming,stored)=>{
  for (const store of ['log','foods','weights','inv','health','xp']) {
    for (const row of incoming[store]) assert.ok(stored[store].some(r=>JSON.stringify(r)===JSON.stringify(row)),
      `missing unrelated ${store} row: ${JSON.stringify(row)}`);
  }
  for (const row of incoming.kv.filter(r=>r.k.startsWith('unrelated-'))) assert.deepEqual(kv(stored)[row.k],row.v);
};
for(const replace of [false,true])for(const direction of [0,1]){
  ...
  if(before){assert.equal(error,'laboratory-restore-conflict');assert.equal(survived,false);failures++;}
    ...
    assertRowsSurvive(incoming,stored);
    if(!replace)assertRowsSurvive(forks[direction],stored);
...
if(before){assert.equal(pushed,false);assert.equal((await D.kvGet('backupFail')).reason,'laboratory-restore-conflict');failures++;}
  ...
  for(const fork of forks)assertRowsSurvive(fork,recovered);
  assert.equal(kv(recovered).labIncubatorRecovery[0].opId,'r6-buy-b');
  ...
  const pulledSave=await D.exportAll();
  for(const fork of forks)assertRowsSurvive(fork,pulledSave);
