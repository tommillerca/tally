...
assert.equal(changed.sheets.length,0,'stale confirmation refuses before opening a review');
assert.match(changed.disclosure,/changed.*Review its losses/);
assert.match(changed.panel,/last Midnight/);
await changed.click();assert.match(changed.disclosure,/last Midnight/);
