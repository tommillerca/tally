...
const ACTIONS = [
  ...
  /* sites went 4 to 2 on 2026-09-05 (offline crash seam OFF-2a): the spend and
     the claim now share ONE db.claimAndPay transaction, so the two REFUND call
     sites (`coinsAdd(price)`/`boneDustAdd(price)` on a lost claim) are gone
     outright, not merely re-shaped: a call that lost its claim under the new
     ordering never touched the balance, so there is nothing to give back. The
     two remaining sites are the same pair as before the 2026-08-31 reorder,
     the recovery grantCosmetic and the main-path grantCosmetic. */
  { id: 'js/loot.js:buyRackItem', sites: 2,
    transition: 'a rack piece goes from unowned to owned, once and forever',
    authority: "db.claimAndPay('kv', {k:'rackbuy:<artId>',...}, {kv}), folding the receipt claim and the balance debit into ONE IndexedDB transaction (2026-09-05). Spend-then-claim in two transactions (2026-08-31 to 2026-09-05) fixed the cross-item overdraft below but left a crash between the two writes able to debit the wallet with no receipt on disk, so a retry after the crash spent again: measured, a 6,000-coin item took 12,000. The kv callback is synchronous and throws to abort the whole transaction when the wallet is short, which rolls the just-added receipt back too, so an insufficient wallet claims nothing and spends nothing, same as spendCoins returning null. It still refuses the cross-item overdraft the 2026-08-31 reorder was for: two DIFFERENT items bought in the same instant each run their OWN claimAndPay transaction, and IndexedDB serialises readwrite transactions on the kv store, so the second one to run sees the first one's real debit rather than a stale read. Measured pre-2026-08-31 on origin/main 2faa73b6: a 3,000-coin wallet took a 3,000 and a 2,400 piece together and kept both, and 160 dust took 160 + 130",
    undriven: "driven to destruction by tests/purchase-firewall.mjs (the second-attempt and cross-item proofs) and tests/unit.test.js R-offseam-2a (the crash-between-writes proof added with this fix). purchase-firewall buys through the real function against a real IndexedDB, measures every store either side, and performs the same purchase twice sequentially AND three times concurrently; proven red there historically on a kvGet/kvSet claim (3 callers charged 7,200 for a 2,400 item) and on paying before the claim. R-offseam-2a stubs db.claimAndPay to reject once for the receipt key, the same trigger tests/purchase-write-failure-audit.mjs uses on db.addIfAbsent, and asserts the retry costs exactly one price" },
