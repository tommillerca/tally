...
const ACTIONS = [
  ...
  { id: 'js/game.js:award', sites: 1, drive: 'award',
    ...
    authority: 'db.add on the xp store: the check and the write are one transaction' },
  ...
  { id: 'js/loot.js:redeemCode', sites: 2, drive: 'redeem',
    ...
    authority: "db.add of the kv row redeemed:<code>, with the legacy 'redeemed' list still read first" },
  ...
  { id: 'js/game.js:grantLevelRewards', sites: 6, drive: 'levelRewards',
    ...
    authority: "the levelup-<L> ledger row's `claimed` flag, because the row itself carries 0 XP" },
