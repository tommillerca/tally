...
await row('LINEAGE', async () => {
  ...
  /* LINEAGE row removed 2026-09-08: it asserted `function petLineageStatsCopy(`,
     which exists in NO branch of this repo (checked main and all five lane
     worktrees). It was written against a refactor that never landed, so it
     could never be true. Restore it with the refactor, not before. */
