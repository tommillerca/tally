...
/* NO DEFAULT URL (Walt). This used to fall back to http://localhost:8765/, which
   in this house is normally another session's checkout, so a green run proved
   nothing about the code in front of you. There is no safe default: say which
   tree. `npm run gate` serves this checkout on its own port and passes it in. */
