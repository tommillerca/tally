#!/usr/bin/env python3
"""Pet morphs v2 (ember / frost / toxic / midnight) for C1-C6.
...
Method (docs/pet-recolor-animation-skill.md, section 2, made region-aware):
  ...
  4. Targets are chosen per region per morph from four palette families, with the
...
MORPHS = ["ember", "frost", "toxic", "midnight"]
...
SPECIES = {
 "C1": dict(name="Drizzle", scale=1, regions={
   "body":  dict(src=["#cffbff"], ember=[hsv(16,.72,.98)], frost=[hsv(200,.32,.96)], toxic=[hsv(82,.72,.95)], midnight=[hsv(252,.55,.50)]),
   "drops": dict(src=["#62e6f2"], ember=[hsv(40,.85,1.0)], frost=[hsv(205,.15,1.0)], toxic=[hsv(95,.85,.60)], midnight=[hsv(220,.35,.85)]),
 ...
 "C2": dict(name="Mallard", scale=1, regions={
   "body":  dict(src=["#9f6b2e"], ember=[hsv(16,.80,.72)], frost=[hsv(203,.35,.85)], toxic=[hsv(82,.80,.85)], midnight=[hsv(250,.55,.45)]),
   "head":  dict(src=["#2eb920"], ember=[hsv(355,.85,.55)], frost=[hsv(215,.65,.62)], toxic=[hsv(135,.85,.50)], midnight=[hsv(270,.55,.55)]),
   "beak":  dict(src=["#ff7536"], ember=[hsv(42,.90,1.0)], frost=[hsv(18,.40,.96)], toxic=[hsv(55,.90,1.0)], midnight=[hsv(235,.40,.78)]),
   "speculum": dict(src=["#4646bd"], ember=[hsv(42,.85,1.0)], frost=[hsv(205,.12,1.0)], toxic=[hsv(65,.80,1.0)], midnight=[hsv(220,.35,.85)]),
 ...
 "C3": dict(name="Catfish", scale=1, regions={
   "body":  dict(src=["#a8a6ff", "#6460fc"],
                 ...
                 toxic=[hsv(82,.55,.96), hsv(88,.88,.68)], midnight=[hsv(250,.50,.58), hsv(252,.72,.42)]),
   "rolls": dict(src=["#ff9692"], ember=[hsv(38,.60,1.0)], frost=[hsv(232,.22,1.0)], toxic=[hsv(55,.55,1.0)], midnight=[hsv(280,.35,.62)]),
   "hilite": dict(src=["#affeff"], ember=[hsv(45,.35,1.0)], frost=[hsv(200,.08,1.0)], toxic=[hsv(65,.28,1.0)], midnight=[hsv(220,.35,.88)]),
   "cream": dict(src=["#fff7d3"], keep=True),      # X eye patch, roll tops
   "white": dict(src=["#ffffe7"], keep=True),
 }),
 "C4": dict(name="Beardie", scale=1, regions={
   "body":  dict(src=["#ff8245"], ember=[hsv(10,.85,.92)], frost=[hsv(203,.45,.90)], toxic=[hsv(92,.85,.72)], midnight=[hsv(250,.65,.50)]),
   "marks": dict(src=["#fcf428"], ember=[hsv(40,.85,1.0)], frost=[hsv(200,.12,1.0)], toxic=[hsv(68,.85,1.0)], midnight=[hsv(220,.35,.85)]),  # stripes + cheek ear-spot
   "belly": dict(src=["#fff9dd"], big=True,        # cream comps >= 500 px = belly; small cream comps (eye whites, teeth) protected
                 ember=[hsv(38,.16,1.0)], frost=[hsv(70,.07,1.0)], toxic=[hsv(60,.14,1.0)], midnight=[hsv(60,.08,.96)]),
   "drool": dict(src=["#b6f0e8"], keep=True),
   "white": dict(src=["#ffffe7"], keep=True),
 }),
 "C5": dict(name="Bulldog", scale=1, regions={
   "body":  dict(src=["#ca906c"], ember=[hsv(14,.72,.80)], frost=[hsv(205,.30,.78)], toxic=[hsv(88,.55,.70)], midnight=[hsv(250,.50,.45)]),
   "stripes": dict(src=["#ff7b5b"], ember=[hsv(40,.80,1.0)], frost=[hsv(205,.55,.88)], toxic=[hsv(65,.80,.95)], midnight=[hsv(225,.40,.80)]),
 ...
 "C6": dict(name="Bumbleseal", scale=3.2, regions={
   "stripes": dict(src=["#ffe100"], ember=[hsv(20,.90,1.0)], frost=[hsv(200,.35,1.0)], toxic=[hsv(85,.90,.95)], midnight=[hsv(250,.65,.62)]),
...
def build(cid, spec, want=MORPHS):
    ...
    entries = [("ink", INK, {m: INK for m in MORPHS}, None)]
    for rname, reg in spec["regions"].items():
        for k, s in enumerate(reg["src"]):
            tg = {m: (s if reg.get("keep") else reg[m][k]) for m in MORPHS}
    ...
    for i, (rname, s, tg, reg) in enumerate(entries):
        if reg is not None and reg.get("big"):
            ...
            entries.append((rname + "_keep", s, {m: s for m in MORPHS}, None))
    ...
    kept_entry = np.array([e[2]["ember"] == e[1] for e in entries])   # protected entries (same for every morph)
...
def sheet(base, res, bg, cell, path, zoom):
...
def main():
    ...
    for cid, spec in SPECIES.items():
        ...
        im, r, ch, meta = build(cid, spec)
        base[cid], res[cid], allchecks[cid], metas[cid] = im, r, ch, meta
        print(cid, {m: {k: v for k, v in c.items()} for m, c in ch.items()}["ember"], "region_px", meta["region_px"], flush=True)
    sheet(base, res, SURFACE2, 490, os.path.join(OUT, "sheet.png"), 2)        # 2x of the 640-canvas size on --surface-2
    sheet(base, res, TODAY_GREEN, 250, os.path.join(OUT, "sheet-today.png"), 1)  # 1x on Today green
    zooms(base, res, os.path.join(OUT, "zooms.png"))
    # table
    ...
    for cid in base:
        ...
        L += [f"## {cid} {spec['name']}", "", ANATOMY[cid], "", "| region | source | ember | frost | toxic | midnight | px (dominant) |", "|---|---|---|---|---|---|---|"]
        for rname, reg in spec["regions"].items():
            for k, s in enumerate(reg["src"]):
                if reg.get("keep"):
                    row = [s] + ["= (kept)"] * 4
        ...
        L.append(f"| ink | {INK} | = | = | = | = | {meta['region_px'].get('ink',0)} |")
