...
ITEMS_VAR = 'BH_ITEMS_ALL'


def read_block(text, var):
    """The `[...]` array literal assigned to `var`, verbatim, or None."""
    m = re.search(r'(?:export )?const ' + var + r' = (\[.*?\n\]);', text, re.S)
    return m.group(1) if m else None


def splice(text, var, block):
    """Replace only `var`'s array literal, leaving every other byte alone."""
    m = re.search(r'((?:export )?const ' + var + r' = )(\[.*?\n\]);', text, re.S)
    if not m:
        raise SystemExit(f'{OUT_DATA}: found no {var} array to merge into')
    return text[:m.start(2)] + block + text[m.end(2):]


def _prior_items():
    """Name, rarity and slot of every item in the CURRENTLY SHIPPED data file.
...
def main():
    ...
    # Seeded from the shipped names, because only NEW ids get generated ones now:
    # a fresh counter would hand the next hat a "#1" that last drop's hat already
    # has. Curated names carry no "#n" and so contribute nothing, which is right.
    counter = {}
    for was in _prior_items().values():
        n = re.search(r'#(\d+)$', was['name'] or '')
        if n and was['slot']:
            counter[was['slot']] = max(counter.get(was['slot'], 0), int(n.group(1)))
    for it in items:
    ...
    # ---- AND NOTHING BELOW THE ITEM ARRAY MAY BE DROPPED EITHER ----
    # The block above keeps every item, and a rebuild still broke the app.
    # Measured against the shipped file: this template emits four exports, but
    # data/boneheadz.js also carries BH_ITEMS_ALL and its `unreleased` filter,
    # BH_ITEMS_WITH_UNRELEASED, PET_SHOP, PET_SLOTS and PET_CROP, all hand-written
    # below the array. A regenerating write deleted every one of them, and they
    # are imported BY NAME in js/app.js, js/loot.js and js/paddock-cards.js, so
    # the module does not merely lose a feature, it fails to load. PET_SHOP is
    # what sells Bumbleseal for 50,000 coins.
    #
    # So the shipped file is EDITED, not rewritten: only the two generated array
    # literals are spliced in, and every other byte, comment and table is carried
    # through untouched. A rebuild with no new art is byte-identical, which is the
    # property that makes it safe to run at all. The template below stays as the
    # bootstrap for a checkout with no manifest yet, the case _prior_items()
    # already returns {} for.
    prev = open(OUT_DATA, encoding='utf-8').read() if os.path.exists(OUT_DATA) else None
    if prev and read_block(prev, ITEMS_VAR):
        fresh = [it for it in items if it['id'] not in prior]
        body = splice(prev, 'BH_SLOTS', json.dumps(slots_out, indent=1))
        if fresh:
            arr = read_block(body, ITEMS_VAR)
            entries = ',\n'.join(' ' + json.dumps(it, indent=1).replace('\n', '\n ') for it in fresh)
            body = splice(body, ITEMS_VAR, arr[:-2] + ',\n' + entries + '\n]')
        print(f'{len(fresh)} new item(s) appended; every other byte left alone')
        for it in fresh:
            print('  + ' + json.dumps(it))
    else:
        body = (
        ...
        print('no manifest to merge into; wrote a fresh one')
