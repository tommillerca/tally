...
- **Talent specs (WoW-style)**: four specs, six nodes each, in tiered trees (paired choices, capstones gated at 5 points in-tree), one point per level, free respec. Slab the Bruiser (lifesteal, SUNDER debuffs, guaranteed staggers, TITAN slam), Greyhound the Runner (+1 AP, counter-jabs, stacking BLEEDS, FLURRY), Ringmaster the Showman (Hype engine, WEAKEN taunts, Second Wind proc, Showstopper), and Gravecaller the Necromancer: Bone Bolt casts at any range scaling off the Hype stat, Mend Marrow heals, Hex curses, life drain, and a BONE STORM capstone. A maxed build takes ~25 levels of real behavior.
