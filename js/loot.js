...
import { db, kvGet, kvSet, kvUpdate, newId } from './db.js';
...
/* ONE TRANSACTION, or the balance loses writes. Read-then-write let two
   overlapping payouts both start from the same balance and the second one win:
   measured 2026-08-17, two concurrent coinsAdd(140) moved the balance by 140
   instead of 280. Same clamp, same return value, no caller changes. */
export async function coinsAdd(n) {
  return kvUpdate('coins', c => Math.max(0, (c || 0) + n), 0);
}
...
export async function boneDustAdd(n) {
  return kvUpdate('bonedust', d => Math.max(0, (d || 0) + n), 0);   // one transaction, same reason as coinsAdd
}
...
export async function redeemCode(raw) {
  ...
  /* THE CLAIM IS A ROW, NOT A LIST. The `redeemed` array above is a
     ...
     window. A per-code kv row claimed with db.add is indivisible, so exactly
     one caller can ever take the code. The list is still read (above) and still
     written (below) so devices that redeemed before this change stay redeemed,
     and it is still what a restore carries. */
  if (!(await db.add('kv', { k: `redeemed:${code}`, v: Date.now() }))) return { ok: false, reason: 'used' };
