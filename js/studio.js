...
export const STUDIO_DEFAULTS = Object.freeze({ backdrop: null, includePet: true,
  caption: 'notes', bubblePosition: 'left', markPosition: 'bottom', stickers: Object.freeze([]), includeFriendCode: false, frame: null, layerOrder: Object.freeze([]) });
...
// Smooth illustration sampling. Canvas high quality blends coverage at scaled edges.
export function studioRasterSticker(figure, sticker, runtime) {
  ...
  ac.save(); ac.translate(pad + artWidth / 2, pad + artHeight / 2);
  ac.rotate((sticker.rotation || 0) * Math.PI / 180);
  ac.scale(scale * (sticker.flip ? -1 : 1), scale);
  ac.drawImage(figure.canvas, b.x, b.y, b.width, b.height, -b.width / 2, -b.height / 2, b.width, b.height);
  ac.restore();
  const pixels = ac.getImageData(0, 0, width, height);
...
// Draft indices are stable through re-renders. Removal explicitly remaps them.
export function studioLayerStack(options) {
  const available = ['pet', 'body', ...(options.stickers || []).map((_, i) => `sticker-${i}`)];
  const saved = Array.isArray(options.layerOrder) ? options.layerOrder : [];
  return [...new Set([...saved.filter(id => available.includes(id)), ...available])];
}
export function studioMoveLayer(options, index, direction) {
  const order = studioLayerStack(options), from = order.indexOf(`sticker-${index}`);
  const to = from + Math.sign(direction);
  if (from >= 0 && to >= 0 && to < order.length) [order[from], order[to]] = [order[to], order[from]];
  options.layerOrder = order;
}
export function studioRemoveSticker(options, index) {
  options.layerOrder = studioLayerStack(options).filter(id => id !== `sticker-${index}`).map(id => {
    if (!id.startsWith('sticker-')) return id;
    const i = Number(id.slice(8)); return `sticker-${i > index ? i - 1 : i}`;
  });
  options.stickers.splice(index, 1);
}
...
export async function studioStickerFigure(sticker, runtime = studioBrowserRuntime(), load = src => runtime.loadImage(src)) {
  ...
  const c = runtime.createCanvas(size, size), cc = c.getContext('2d'); cc.imageSmoothingEnabled = true; cc.imageSmoothingQuality = 'high';
  for (let i = 0; i < images.length; i++) {
    const layer = runtime.createCanvas(size, size), lc = layer.getContext('2d'); lc.imageSmoothingEnabled = true; lc.imageSmoothingQuality = 'high';
...
export async function composeStudio(look, options = {}, runtime = studioBrowserRuntime()) {
  ...
  const background = runtime.createCanvas(1080, 1920);
  ...
  const paint = (figure, box, flip = false) => {
    ...
    if (box.id === 'body' || box.id === 'pet') {
      const surface = runtime.createCanvas(1080, 1920), sc = surface.getContext('2d');
      sc.imageSmoothingEnabled = true; sc.imageSmoothingQuality = 'high';
      sc.drawImage(figure.canvas, b.x, b.y, b.width, b.height, box.x, box.y, box.width, box.height);
      sceneLayers[box.id] = surface;
    }
  };
  ...
  for (const [index, sticker] of plan.stickers.entries()) {
    ...
    const surface = runtime.createCanvas(1080, 1920), sc = surface.getContext('2d');
    sc.drawImage(raster.decoration, box.x, box.y); sc.drawImage(raster.art, box.x, box.y);
    sceneLayers[box.id] = surface; bounds.push(box);
  }
  const layerOrder = studioLayerStack(options);
  ctx.clearRect(0, 0, 1080, 1920); ctx.drawImage(background, 0, 0);
  for (const id of layerOrder) if (sceneLayers[id]) ctx.drawImage(sceneLayers[id], 0, 0);
  bounds.sort((a, b) => layerOrder.indexOf(a.id) - layerOrder.indexOf(b.id));
  ...
  let markContrast = studioMarkContrast(before, ctx.getImageData(0, 0, 1080, 1920).data,
  ...
  // Flatten the information once, identically for the export and live gestures.
  main.imageSmoothingQuality = 'low'; // Exact-size surface copies do not need resampling.
  main.clearRect(0, 0, 1080, 1920); main.drawImage(background, 0, 0);
  for (const id of layerOrder) if (sceneLayers[id]) main.drawImage(sceneLayers[id], 0, 0);
  main.drawImage(foreground, 0, 0);
  markContrast = studioMarkContrast(before, main.getImageData(0, 0, 1080, 1920).data,
    mc.getImageData(0, 0, 1080, 1920).data);
  assertStudioMarkContrast(markContrast);
  const blob = await runtime.encode(canvas);
  await assertStudioPng(blob);
  const renderLive = (target, stickers) => {
    const cx = target.getContext('2d'); cx.imageSmoothingEnabled = true; cx.imageSmoothingQuality = 'low';
    cx.clearRect(0, 0, 1080, 1920); cx.drawImage(background, 0, 0);
    const liveBounds = bounds.filter(b => !b.id.startsWith('sticker-'));
    for (const id of layerOrder) {
      if (!id.startsWith('sticker-')) { if (sceneLayers[id]) cx.drawImage(sceneLayers[id], 0, 0); continue; }
      const index = Number(id.slice(8)), sticker = stickers[index];
      if (!figures[index] || !sticker) continue;
      const r = studioRasterSticker(figures[index], sticker, runtime), b = { id, ...r.box };
      cx.drawImage(r.decoration, b.x, b.y); cx.drawImage(r.art, b.x, b.y); liveBounds.push(b);
    }
    liveBounds.sort((a, b) => layerOrder.indexOf(a.id) - layerOrder.indexOf(b.id));
    cx.drawImage(foreground, 0, 0);
    return liveBounds;
  };
  return { blob, plan, information, bounds, feetY, markContrast, renderLive, figures, layerOrder, background, sceneLayers, foreground };
}
