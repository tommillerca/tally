...
export const CHANGES = [
  { n: 588, date: '2026-09-12', title: 'Room for the whole name', items: [
    'Long names wrap on podiums, leaderboards, race results, gifts and featured nameplates.',
  ] },
  ...
  { n: 470, date: '2026-08-31', title: 'The Pit remembers your fight, and your account is truly yours', items: [
    ...
    'Rewards that are meant to happen once now happen once, even if you are impatient or have two tabs open. A finished dish could be collected twice off one set of ingredients, a single Pit charge could open two staked fights, and a daily quest cap of three could pay four if you tapped two Claim buttons at the same moment. All three now decide and pay in one indivisible step.',
    'Levelling up from a food log shows ONE celebration instead of two, the second of which used to say you had earned nothing. Your rewards were always correct; the extra sheet was not.',
    'Buying things is now honest to the coin. A fast double tap could charge you twice for one cosmetic, or, worse, take your entire wallet for a single item; and once a wallet hit zero a burst of taps could mint shop items for free. Every purchase path in the game now takes payment as one indivisible step, so you are charged exactly once for exactly what you get, and a purchase you cannot afford leaves your balance untouched. Wearing a look you already paid for can never charge you again.',
    'The app stops going quiet on you. A cloud backup that cannot save now tells you why instead of looking identical to a healthy one: it will say if this save has outgrown its slot, or if this device\u2019s clock is wrong and everything is waiting on that. Your streak pill shows your real streak instead of stopping at 56. Coin amounts in messages read 1,234,477 rather than 1234477. And every kind of paused daily reward now explains its own cause rather than sharing one generic line.',
    'A big accessibility pass. The wallet chips, day arrows, settings toggles, quest Claim buttons and shop price buttons were all far below the size a phone should ask you to hit, some as small as 16 pixels tall, and they now meet the 44 pixel standard without anything moving or looking different. A toast message used to sit invisibly over the buttons underneath it and swallow your taps for three seconds. Text that was too faint to read against its background has been darkened. Wardrobe tiles now say their rarity in words instead of only in a border colour, so it can be read without relying on colour vision. And each fight button now names its own opponent for screen readers, where before there were five buttons all called FIGHT.',
    'Sending a cheer or a gift is now exactly as many as you meant to send, even on a bad connection that makes you tap again, and a captured request can no longer be replayed against your account.',
    'Cloud backup was quietly broken for anyone who had been playing a while, and is fixed. Once a save grew past roughly a few weeks of play, every attempt to upload it failed and said nothing: no warning, no retry, and the backup date simply never moved. If that was you, your save starts backing up again the next time the app opens with backup switched on.',
    'The Paddock field is a place again rather than a pile. Your animals had been packing into each other because three different bits of the scene were placing figures without seeing one another, so pets stood inside pets and the right half of the field sat empty. There is one layout now: everyone gets their own space, the panel tells you honestly how many are out today and how many are resting instead of quietly showing fewer than you own, tapping a tile lights that tile and opens the card next to your thumb instead of across the screen, tapping your own keeper no longer opens a pet you cannot see, and the founders\' teaser stops showing itself to founders.',
    'Restoring an account is honest about what happened now. A connection that drops partway through can no longer leave your phone signed in as the restored account with none of its data: nothing on the device changes until the server has accepted the account, and a save that fails to download says so and retries the next time you open the app instead of telling you no save exists. And a cheer that loses its answer on a bad connection now counts as the one cheer you meant to send, however many times you tap it.',
    'The Paddock finally tells you the truth. The affection hearts have been showing every pet as fully bonded no matter how much you had actually petted them, so the meter now fills as you earn it. Petting your best friend no longer closes the card instead of petting them, a pet napping on the egg nest no longer blocks it, tapping the same pet again closes its card, the dots under a card with several copies of one pet are visible and take you to the copy you press, tapping a specific pet in the field opens that exact pet, a ready egg reads as ready and tells you to open it in your Backpack, and an empty nest stops drawing an egg that is not there.',
    'Coming back after a long time away is kinder now. The day-close crate you earned on your last logged day is waiting instead of quietly gone, a Claim button that cannot pay yet tells you why instead of doing nothing when you tap it, and the streak reminder no longer nags a streak that already ended.',
    'Cloud backup got a serious fix for anyone who plays on two devices. A save could end up locked under a key the other device did not hold, and the app would tell you no save existed at all. Backups now carry the right key from the very first one, restoring on a second device can never overwrite that key, and a backup that genuinely cannot be read says so plainly instead of pretending it is not there.',
  ] },
