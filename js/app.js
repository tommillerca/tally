...
async function openCelebration({ levelUp = null, levelRewards = null, newBadges = [], streakMilestone = null, fromLevel = null, note = null }) {
  ...
  /* No confetti here. The level-up has its own sparkles and lime burst, and
     confetti was retired game-wide in v322 anyway (Tom: "remove the confetti it's
     kinda lame"), so confettiRain() is an early return in js/fx.js. The branch
     this replaced would have read as "badges still get confetti", which is no
...
/* Test seam, webdriver-gated like __packReveal / __spireSheet. The moment is
   reachable only by actually crossing a level, and four of the five bugs its
   author found by hand were about the NUMBERS it prints (which level you came
   from, what the XP bar reads) rather than the motion. Those render a perfectly
   good-looking screen while being wrong, so tests/levelup-audit.mjs drives this
   directly to read them back. Never exists for a player. */
...
if (typeof window !== 'undefined' && navigator.webdriver) window.__levelUpMoment = openLevelUpMoment;
    ...
    if (ev.t === 'hit') {
      ...
      if (arena && stage) {
        import('./wraith-fx.js').then(fx => {
          const anchors = fx.anchorsFor(fx.plateRect(stage), arena.getBoundingClientRect(), el('youStage')?.getBoundingClientRect()); // plateRect: anchors are fractions of the drawn INK, not the letterboxed stage box
