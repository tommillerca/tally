...
function petPortraitHtml(petId, px, shiny = false) {
  if (petId === 'CX') shiny = false; // Founder's Lizard: amethyst CX.png is the portrait (no shiny static)
...
function openSurveySheet(source = 'auto') {
  ...
  openSheet(`
    <div class="survey" id="surveyForm">
      ...
      <textarea id="svWant" class="survey-in" rows="2" maxlength="280" placeholder="If I built one thing next, it should be..."></textarea>
      <p class="survey-priv">Goes straight to the developer, never shared or shown to other players. <a href="privacy.html" target="_blank" rel="noopener">Privacy</a></p>
  ...
  $('#svSend')?.addEventListener('click', async () => {
    ...
    sendSurvey({ name, email, emailOptin: optin, feedback, mostWanted }); // fire-and-forget
    trackEvent('survey_submit', { optin: optin ? 1 : 0, hasEmail: email ? 1 : 0 });
    showFounderReveal(granted);
  });
}

// The celebratory reveal after a survey submit: swaps the form for a big animated
// Founder's Lizard and a thank-you. Replaces the sheet body in place.
...
// — not for brand-new players (need a few sessions to form an opinion), never over
// onboarding / the splash / the daily wheel / any open sheet (retries next boot),
// snoozes on "Maybe later", and NEVER returns once submitted. Skips webdriver/demo.
async function maybeShowSurvey() {
  try {
    if (navigator.webdriver || !S.settings) return;
    if (await kvGet('surveyDone', false)) return;
    // engagement gate: only from the 3rd session on (past first-run onboarding)
    const boots = (await kvGet('surveyBoots', 0)) + 1;
    await kvSet('surveyBoots', boots);
    if (boots < 3) return;
...
async function renderSettings(el) {
  ...
  <div class="card">
    ...
    ${surveyDone ? '' : `<div class="settings-row"><div class="lab"><b>Founding survey 💜</b><span>Share your thoughts, keep the exclusive Founder's Lizard</span></div><button class="btn small" id="surveyBtn" style="background:#b96cf0;color:#1a0f26">Claim</button></div>`}
