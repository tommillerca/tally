...
// Football kit, 2026-09-04: the generated arrays are spliced by the builder; the
// import below and the push after BH_ITEMS_ALL are hand-written and survive it.
import { FOOTBALL_ITEMS, footballTints } from './football-teams.js';
...
/* Football kit, 2026-09-04: 32 teams x 8 garments, generated in
   data/football-teams.js and joined here so every catalogue reader (rack, crates,
   gear, Looks, the renderers) sees one list. They arrive `unreleased` until
   FOOTBALL_KIT_LIVE flips, which is the same gate the line below has always applied. */
BH_ITEMS_ALL.push(...FOOTBALL_ITEMS);
...
export const PET_SLOTS = [
  ...
  /* Football kit, 2026-09-04: the lizard's helmet and jersey. Jersey under
     everything, helmet under the glasses (CE stays the strict top, Tom's rule). */
  { code: 'CT', label: 'Jersey', z: 5 },
  { code: 'CH', label: 'Helmet', z: 35 },
];
...
/* Football kit, 2026-09-04: an accessory names the species it is drawn for
   (`pets`), and one that does not is Bumbleseal's. petWornItems is the shared
   filter; petWornLayers keeps its URL contract for the two renderers that stack
   plain <img>s, and petWornTints hands them the football multiply layers. */
export const petCanWear = (item, petId) => item.pets ? item.pets.includes(petId) : petId === PET_SHOP.pet.id;
export function petWornItems(petId, wear) {
  if (!wear) return [];
  return [...PET_SLOTS].sort((a, b) => a.z - b.z)
    .map(sl => wear[sl.code])
    .filter(id => id && BH_BY_ID[id] && petCanWear(BH_BY_ID[id], petId))
    .map(id => BH_BY_ID[id]);
}
export function petWornLayers(petId, wear) {
  return petWornItems(petId, wear).map(bhAsset);
}
/* Per worn layer, in the same order: the tint layers to draw over it, or null. */
export function petWornTints(petId, wear) {
  return petWornItems(petId, wear).map(footballTints);
}
