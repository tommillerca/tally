...
| F21 | Steps | Tom, 2026-09-12, on the race week boundary (server finding #4): "D6 make it local" | **RULED, unbuilt.** Race weeks become player-local; the Worker must accept the phone's local week key. Server change, deploy is Tom's | `docs/BUGS-v588-server.md` #4 |
| F22 | Server | Tom, 2026-09-12: "D5 podium settlement then fix the rest of the bugs" | **#1 BUILT (v589 train, deploy pending), rest queued.** Build order: podium settlement atomicity (#1), then replay identity (#2), flagged friends (#5), cheer retry key (#10). Built in lanes with the in-process Worker harness; deploy is Tom's | `docs/BUGS-v588-server.md` |
| F23 | Economy | Tom, 2026-09-12: "D4  more explanation needed" on `docs/BUGS-v588-economy.md` | Tom, 2026-09-12: "d4 i think we need to do them all?" **RULED: fix all six.** Lanes l-eco5, l-eco6, l-eco2 building; #1, #3, #4 queued behind them | `docs/BUGS-v588-economy.md` |
