...
| F5 | Melt | "no sound on melt it's also a fucking nightmare that needs to be redesigned basically unuseable with the interface" | Tom, 2026-09-12: "maybe the bench should be in wardrobe? i dont know why it would be in the ktichen if it has to do with gear." **DIRECTION GIVEN, unbuilt:** the Salvage Bench moves to the Wardrobe. Every advertised Salvage Bench route opens the Backpack instead; the bench is below the Kitchen. Sound hooks all exist | `docs/melt-review.md` |
| F6 | Names | Long names overflow narrow names and 200% text sites | **BUILT IN LANE, browser proof blocked.** Round 2 adds within-word wrapping to newcomers and Pit venue, bounded font sizing and wrapping to the Pit HUD, and repairs race/request fixture readiness. REACH includes newcomers, Pit HUD, Pit venue, active race lanes, standing, gap and previous champion. Map sites are explicitly excluded because the fixture does not boot WebGL. Local audit exits 97 on listen EPERM before measurement; green and negative-control browser proofs remain pending. | `tests/name-fit-audit.mjs`, `docs/CLAIMS.md` v588 |
...
| F16 | Logging | from `docs/BUGS-v580.md`: meal chip guesses from the last entry; run 5 measured 60 of 80 meals needing a corrective tap | Tom, 2026-09-12: "explain f16 more idk what you mean". **OPEN — DECISION**, explanation sent, awaiting keep or change | `docs/BUGS-v580.md`, R5-L1 |
| F17 | Logging | from `docs/BUGS-v580.md`: deleting a logged food has no confirm, no name in the message, no undo | Tom, 2026-09-12: "f17 i think just a confirm would be fine". **RULED, unbuilt:** a confirm naming the food, no undo | `docs/BUGS-v580.md`, R5-B6 |
| F7 | Pets | "Are we still having pets grow with breeding or something like that physically? My lvl 1 fish seems small but if that's intentional all good" | Tom, 2026-09-12: "i think pets should grow a little bit with breeding it's a nice twist and reward". **RULED, FEATURE, unbuilt.** Pets grow a little with breeding (render-time uniform scale only, never on disk) | — |
