...
| F18 | Pets | Tom, 2026-09-12: "i never agreed to walks you log by hand adding to eggs people are going to scam that? that is a decision you need to ask me first" | **RULED, reverted in v585.** His decision: "everyones phone can count steps these days if they dont wanna connect it or carry their phone on a walk tough luck." The underlying complaint (no step source means no second pet) is CLOSED by that ruling, not outstanding | `docs/CLAIMS.md` v585 |
| F16 | Logging | from `docs/BUGS-v580.md`: meal chip guesses from the last entry; run 5 measured 60 of 80 meals needing a corrective tap | Tom, 2026-09-12: "explain f16 more idk what you mean". Tom, 2026-09-12: "d1 ya i guess option 3 is best". **BUILT, v590 train:** last meal wins for two hours, then the clock | `docs/BUGS-v580.md`, R5-L1 |
| F7 | Pets | "Are we still having pets grow with breeding or something like that physically? My lvl 1 fish seems small but if that's intentional all good" | Tom, 2026-09-12: "i think pets should grow a little bit with breeding it's a nice twist and reward". **RULED, BUILT IN LANE l-petgrow, HELD.** Render-time uniform scale, +4% per lineage tier to +24%, art untouched. Held out of v588 because the Stable draws pets through the pet card slider (locked file region), so growth would show in the Pit and friend profiles but not where Tom looked. Tom, 2026-09-12: "d2 the pets dont need to grow how they are viewed in the stable just elsewhere like combat scenes, paddock, today tab, crew tab etc." **RULED: not the Stable; the Pit, Today, Crew and Paddock.** Paddock needs Tom to lift the js/paddock.js lock first; the other three plus the app.js Paddock scene are on the v589 train | — |
| F20 | Crew | Tom, 2026-09-12: "there is way too much UTC timezone chatter text on the leader board now. idk when this changed but the leaderboard needs to be simplified the average player doesnt care the exact time someone logged on it should just say \"logged on 2 hours ago or 2 days ago etc\"" | **BUILT, v589 train.** The timestamp arrived in v571 (`leaderboardLastOnline`); now "online now", "N hours ago", "yesterday", "N days ago" | — |
| F21 | Steps | Tom, 2026-09-12, on the race week boundary (server finding #4): "D6 make it local" | **BUILT, v590 train, Worker half pending Tom's deploy.** Local calendar Fridays on the phone; the Worker accepts a key within one day of its UTC period start | `docs/BUGS-v588-server.md` #4 |
| F22 | Server | Tom, 2026-09-12: "D5 podium settlement then fix the rest of the bugs" | **ALL BUILT (#1 in v589, #2 #5 #10 on the v590 train), one deploy bundle pending Tom.** Build order: podium settlement atomicity (#1), then replay identity (#2), flagged friends (#5), cheer retry key (#10). Built in lanes with the in-process Worker harness; deploy is Tom's | `docs/BUGS-v588-server.md` |
| F23 | Economy | Tom, 2026-09-12: "D4  more explanation needed" on `docs/BUGS-v588-economy.md` | Tom, 2026-09-12: "d4 i think we need to do them all?" **ALL SIX BUILT, v590 train.** Gifts and spire coins needed Worker changes too; those wait on Tom's deploy | `docs/BUGS-v588-economy.md` |
...
| F9 | Onboarding | The restore-button trap: a non-gamer burned 14 taps in a restore sheet after taking the primary-styled "Restore an account" | Tom, 2026-09-12: "d3 agreed with gwart picks (love that name for this feature btw) swap them it should be new player focused". **BUILT, v589 train:** on the unknown/no-save branch "I am new" becomes primary and Restore the ghost; recovery-first stays when a save is known to be missing | run 9 |
...
|---|---|---|---|
| Wardrobe | The Salvage Bench moves out of the Backpack to the bottom of the Wardrobe; every "Salvage Bench" shortcut lands there (F5, Tom: "maybe the bench should be in wardrobe? i dont know why it would be in the ktichen if it has to do with gear.") | v588 | `melt-ui-audit` rows HOME, GONE, ROUTE green on the train, red on live v587; `wardrobe-stack-audit` caught round 1 hiding the Dressing Room apply button, round 2 passes it |
| Logging | Deleting a logged food asks you to confirm its name (F17, Tom: "f17 i think just a confirm would be fine") | v588 | `log-delete-confirm-audit` 7 rows green on the train, ASKS red on live v587 |
...
| Wardrobe | One slot at a time, the two nav buttons separated, rules in a dropdown | v586 | `wardrobe-stack-audit` retargeted; 12 sections to 0; buttons 361px flush to 101/109px 18px apart |
