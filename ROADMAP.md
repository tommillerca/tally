...
| F6 | Names | Names are being cut off on leaderboard, podium, race and gift rows | **PARTIAL, v588 implementation awaiting browser proof.** Builder-length fixtures, rendered inline controls and scroll-axis checks replace the misleading diagnostic. Scoped names now wrap | `tests/name-fit-audit.mjs` (full), `docs/CLAIMS.md` v588 |
